...
**原始碼版本：`v0.4.8`——一般任務無預設時限及 RAM 快取。** 請閱讀[版本說明](docs/releases/v0.4.8.md)及[指令／快取契約](docs/long-running-commands.md)。原始碼中的版本號不代表發佈建置已通過；只應安裝已公開且有驗證證據的檔案，本工具並非經認證的安全沙箱。
