...
const releaseVersion = process.env.RELEASE_VERSION ?? '0.7.0-rc.12';
...
const verificationRenderer = path.join(aiTemp, 'rc12-release', 'renderer-dist', sourceSha);
const npmCache = path.join(aiTemp, 'cache', 'npm-release-rc12');
...
const childEnv = {
  ...
  VALIDATION_WORKFLOW: '.github/workflows/codex-router-multiprovider-release-rc12.yml',
  ...
  BUN_INSTALL_CACHE_DIR: path.join(aiTemp, 'cache', 'bun-release-rc12'),
...
async function main() {
  ...
  assert(releaseVersion === '0.7.0-rc.12', `RELEASE_VERSION_MISMATCH:${releaseVersion}`);
  assert(releaseTag === 'v0.7.0-rc.12', `RELEASE_TAG_MISMATCH:${releaseTag}`);
  ...
  run('pwsh', ['-NoLogo', '-NoProfile', '-File', 'aiTemp/rc12-release/verify-windows-migration.ps1',
  ...
  const provenance = {
    ...
    validation_run: { id: validationRunId, url: validationRunUrl, workflow: '.github/workflows/codex-router-multiprovider-release-rc12.yml' },
