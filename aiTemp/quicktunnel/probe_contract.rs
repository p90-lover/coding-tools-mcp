...
async fn fixture_doc(
    State(s): State<ProbeFixture>,
    ...
    let value = match uri.path() {
        "/mcp" => {
            json!({"name":"coding-tools-mcp","version":"fixture","protocolVersion":"2025-11-25"})
        }
...
async fn quick_connection_pool_parallelism_and_credential_boundaries() {
    ...
    settings.proxy.mode = "none".into();
    for _ in 0..4 {
        document(&client(&settings).unwrap(), &format!("{origin}/mcp"))
            .await
            .unwrap();
    }
    ...
    probe_with_client(&origin, TunnelServiceKind::Mcp, true, &c)
        .await
        .unwrap();
    let parallel_requests = fixture.peak.load(Ordering::SeqCst);
    let mut serial_ms = vec![];
    let mut parallel_ms = vec![];
    for _ in 0..5 {
    ...
    probe_with_client(&origin, TunnelServiceKind::Actions, true, &c)
        .await
        .unwrap();
    let mut invalid = settings.clone();
    invalid.proxy.mode = "manual".into();
    invalid.proxy.url = "http://[".into();
    assert!(
        client(&invalid).is_err(),
        "Invalid changed proxy must not reuse a prior direct client"
    );
    ...
    server.await.unwrap();
    println!(
        "QUICK_PROBE_EVIDENCE {}",
        json!({"pooled_connections":pooled_connections,"parallel_requests":parallel_requests,"serial_ms":serial_ms,"parallel_ms":parallel_ms,"fixture_delay_ms":40,"sample_count":5,"public_network_measured":false,"no_credentials_sent":true})
    );
