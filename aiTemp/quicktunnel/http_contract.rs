...
async fn check(root: PathBuf) {
    ...
    let state = ListenerState {
        ...
        bearer_token: Some("synthetic-control-token".into()),
        oauth: None,
    ...
    for method in ["ping", "initialize", "tools/list"] {
        ...
        assert_eq!(
            client
                .post(&url)
                .bearer_auth("synthetic-control-token")
        ...
        assert_eq!(
            response.status(),
            200,
            "CONTROL_BLOCKED_BY_BUSY_TOOL_WORKERS"
        );
        let value: Value = response.json().await.unwrap();
        assert_eq!(value["id"], method);
        assert!(value.get("error").is_none());
        latencies.push(
            json!({"method":method,"milliseconds":start.elapsed().as_micros() as f64/1000.0}),
        );
    ...
    let modern_response = client
        ...
        .await
        .unwrap();
    assert_eq!(modern_response.status(), 200);
    assert!(modern_response
        .json::<Value>()
        .await
        .unwrap()
        .get("error")
    ...
    for tool in ["server_info", "ping"] {
        ...
        assert_eq!(
            client
                ...
                .status(),
            503,
            "A tool name must not bypass execution admission"
        );
    }
    ...
    let status = client
        ...
        .unwrap();
    assert_eq!(
        status["result"]["structuredContent"]["operations"],
        json!([])
    );
    ...
    println!(
        "QUICK_HTTP_EVIDENCE {}",
        json!({"latencies":latencies,"occupied_execution_slots":16,"auth_origin_protocol_checked":true,"read_only_profile_retained":true,"tool_execution_not_retried":true,"public_network_measured":false})
    );
}
