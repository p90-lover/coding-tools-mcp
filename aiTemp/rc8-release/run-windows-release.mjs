...
const releaseVersion = process.env.RELEASE_VERSION ?? '0.7.0-rc.8';
...
const verificationRenderer = path.join(aiTemp, 'rc8-release', 'renderer-dist', sourceSha);
...
const childEnv = {
  ...
  BUN_INSTALL_CACHE_DIR: path.join(aiTemp, 'cache', 'bun-release-rc8'),
  TMPDIR: temp,
  TMP: temp,
  TEMP: temp,
...
async function main() {
  ...
  assert(releaseVersion === '0.7.0-rc.8', `RELEASE_VERSION_MISMATCH:${releaseVersion}`);
  assert(releaseTag === 'v0.7.0-rc.8', `RELEASE_TAG_MISMATCH:${releaseTag}`);
  ...
  run('pwsh', ['-NoLogo', '-NoProfile', '-File', 'aiTemp/rc8-release/verify-windows-migration.ps1',
  ...
  const provenance = {
    ...
    validation_run: { id: validationRunId, url: validationRunUrl, workflow: '.github/workflows/codex-router-multiprovider-release-rc8.yml' },
  ...
  run('node', ['scripts/release/publish-v0.6.0-rc.1.mjs', '--repository', repository, '--tag', releaseTag, '--source', sourceSha, '--assets', assets, '--notes', 'docs/releases/v0.7.0-rc.8.md', '--json-out', path.join(evidence, 'publication.json')], { label: 'publish and read back rc.8 prerelease' });
  ...
  process.stdout.write(`\nRC8_WINDOWS_RELEASE_PUBLISHED ${releaseTag} ${sourceSha} ${installerSha}\n`);
}
