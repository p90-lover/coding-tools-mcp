...
const releaseVersion = process.env.RELEASE_VERSION ?? '0.7.0-rc.10';
...
const verificationRenderer = path.join(aiTemp, 'rc10-release', 'renderer-dist', sourceSha);
...
const childEnv = {
  ...
  VALIDATION_WORKFLOW: '.github/workflows/codex-router-multiprovider-release-rc10.yml',
  ...
  BUN_INSTALL_CACHE_DIR: path.join(aiTemp, 'cache', 'bun-release-rc10'),
  TMPDIR: temp,
  TMP: temp,
  TEMP: temp,
...
async function main() {
  ...
  assert(releaseVersion === '0.7.0-rc.10', `RELEASE_VERSION_MISMATCH:${releaseVersion}`);
  assert(releaseTag === 'v0.7.0-rc.10', `RELEASE_TAG_MISMATCH:${releaseTag}`);
  assert(repository, 'GITHUB_REPOSITORY_MISSING');
  assert(branch === 'release/codex-router-multiprovider-0.7.0-rc.8', `RELEASE_BRANCH_MISMATCH:${branch}`);
  ...
  ], { label: 'focused Provider Center, Paseo, Anneal, proxy, localization, and rc.10 identity contracts' });
  ...
  run('pwsh', ['-NoLogo', '-NoProfile', '-File', 'aiTemp/rc10-release/verify-windows-migration.ps1',
  ...
  const provenance = {
    ...
    validation_run: { id: validationRunId, url: validationRunUrl, workflow: '.github/workflows/codex-router-multiprovider-release-rc10.yml' },
  ...
  run('node', ['scripts/release/publish-v0.6.0-rc.1.mjs', '--repository', repository, '--tag', releaseTag, '--source', sourceSha, '--assets', assets, '--notes', 'docs/releases/v0.7.0-rc.10.md', '--json-out', path.join(evidence, 'publication.json')], { label: 'publish and read back rc.10 prerelease' });
  ...
  process.stdout.write(`\nRC10_WINDOWS_RELEASE_PUBLISHED ${releaseTag} ${sourceSha} ${installerSha}\n`);
}
