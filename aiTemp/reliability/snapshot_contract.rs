...
fn reliability_snapshot_rejected_inputs_do_not_consume_retention() {
    ...
    let base = std::env::current_dir()
        .unwrap()
        .join("aiTemp/reliability-snapshots")
    ...
    let count = |home: &Path| {
        fs::read_dir(home.join("runs"))
            .map(|r| r.count())
            .unwrap_or(0)
    };
    for names in [
        vec!["../private.txt"],
        vec!["good.txt", "missing.txt"],
        vec!["nested"],
        vec!["auth.json"],
        vec!["good.txt", "good.txt"],
        vec!["nested/input.txt", "nested/./input.txt"],
    ] {
    ...
    let run = snapshot::prepare(
        &root,
        &storage,
        &["good.txt".into(), "nested/input.txt".into()],
    )
    .unwrap();
    assert_eq!(count(&storage), 1);
    assert_eq!(
        fs::read(run.input.join("good.txt")).unwrap(),
        b"SOURCE_UNCHANGED"
    );
    assert_eq!(
        fs::read(run.input.join("nested/input.txt")).unwrap(),
        b"NESTED_UNCHANGED"
    );
    assert_eq!(
        fs::read(root.join("good.txt")).unwrap(),
        b"SOURCE_UNCHANGED"
    );
    assert!(run.work.is_dir());
    for _ in 1..32 {
        snapshot::prepare(&root, &storage, &[]).unwrap();
    }
    assert!(
        snapshot::prepare(&root, &storage, &[]).is_err(),
        "The retained-run limit must remain enforced"
    );
