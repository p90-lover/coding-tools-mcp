...
fn reliability_task_events_schema_and_capability_discovery() {
    ...
    let directory = std::env::current_dir()
        .unwrap()
        .join("aiTemp/reliability-metadata")
        .join(uuid::Uuid::new_v4().to_string());
    std::fs::create_dir_all(directory.join("workspace")).unwrap();
    let mut ctx =
        ToolContext::for_test(directory.join("workspace"), directory.join("harness")).unwrap();
    ctx.tool_profile = "advanced".into();
    let definition = registry::list_tools()
        .into_iter()
        .find(|v| v["name"] == "list_task_events")
        .unwrap();
    assert!(
        definition["outputSchema"].is_object(),
        "TASK_EVENTS_OUTPUT_SCHEMA_MISSING"
    );
    let task = ctx
        .harness
        .start_task("Synthetic schema fixture; no agent or provider request")
        .unwrap();
    ctx.harness
        .record_event(
            &task.id,
        ...
        .unwrap();
    let first = call_tool_mcp(
        &ctx,
        "list_task_events",
        &json!({"task_id":task.id,"limit":1}),
    );
    assert_eq!(first["ok"], true);
    assert_eq!(first["events"].as_array().unwrap().len(), 1);
    let next = call_tool_mcp(
        &ctx,
        "list_task_events",
        &json!({"task_id":task.id,"cursor":first["next_cursor"],"limit":200}),
    );
    assert_eq!(next["ok"], true);
    let empty = call_tool_mcp(
        &ctx,
        "list_task_events",
        &json!({"task_id":task.id,"cursor":10000,"limit":1}),
    );
    ...
    for profile in ["advanced", "core", "read-only"] {
        ...
        for name in ["sandbox_status", "sandbox_exec", "mcp_operation_status"] {
            assert_eq!(
                reported.contains(&json!(name)),
                exposed.contains(&name),
                "CAPABILITY_REPORT_OMITS_RELEASED_TOOL {profile}/{name}"
            );
        }
    ...
    println!(
        "RELIABILITY_SCHEMA_EVIDENCE {}",
        json!({"definition":definition,"samples":[first,next,empty,invalid]})
    );
