"""Stable publication: exact source, bounded scope, retained gates and asset readback."""
from pathlib import Path
import hashlib,json,os,shutil,subprocess,time,zipfile
REPO='p90-lover/coding-tools-mcp';BASE='2517b2e7fe0ae2038eb3b72caa8761756927feb4'
BRANCH='fix/task-monitor-latest-0.4.4';VERSION='0.4.5';TAG='v'+VERSION
...
assert not api('git/matching-refs/tags/'+TAG),'Do not overwrite an existing release'
assert api('git/ref/heads/main')['object']['sha']==BASE,'Concurrent main changed; reconcile before publication'
assert api('git/ref/heads/'+BRANCH)['object']['sha']==SOURCE
root=Path('aiTemp/task-monitor-publication');root.mkdir(parents=True,exist_ok=False)
download('monitor-installer-'+RUN,root/'installer');download('monitor-evidence-'+RUN,root/'evidence')
...
for field in ['task_monitor_history_group_passed','update_semver_precedence_passed','monitor_read_only','stable_latest_intended','context_budget_group_passed','canonical_linked_write_group_passed','root_nonoverwrite_verified','workspace_refresh_coalescing_passed','operation_cache_bound_to_root_revision']:
    assert proof[field] is True,field
for field in ['monitor_initializes_history','monitor_scans_project','raw_tool_payloads_in_monitor','dispatch_completion_is_job_completion','automatic_operation_replay','source_panels_assign_or_control_agents','full_codex_paseo_anneal_orchestration_implemented','live_user_account_verified','automatic_linking_of_all_profiles','baseline_partial_acceptance','stored_task_or_event_truncated']:
    assert proof[field] is False,field
for filename,field in [('task-monitor-browser.json','task_monitor_browser'),('source-details-browser.json','source_details_browser'),('refresh-browser.json','live_refresh_browser')]:
    ui=json.loads((root/'evidence'/filename).read_text());assert ui==proof[field]
    assert ui['source']==SOURCE and ui['passed'] and ui['model_requests']==0 and not ui['console_errors']
assert proof['source_details_browser']['mutations']==0
for name,count in [('task-monitor-history.txt',1),('update-green.txt',1),('live-tests.txt',2),('context-green.txt',1),('linked-write-green.txt',1),('quick-tests.txt',3),('recovery-tests.txt',3),('oauth-tests.txt',5),('snapshot-contract.txt',3)]:
...
provenance={'version':VERSION,'source_commit':SOURCE,'main_commit':SOURCE,'workflow_run':int(RUN),'preserved_main':BASE,
    'windows':proof,'extension_version':'0.0.7','live_user_connection_verified':False,'model_requests':0,'release_channel':'stable','full_upstream_function_parity':False}
...
release=api('releases',{'tag_name':TAG,'target_commitish':SOURCE,'name':'Coding Tools MCP '+TAG+' — task monitor and current stable update','body':notes,'draft':True,'prerelease':False,'make_latest':'false'})
Path('aiTemp/monitor-release-draft.json').write_text(json.dumps({'release_id':release['id'],'source':SOURCE}))
...
public=api('releases/'+str(release['id']),{'draft':False,'prerelease':False,'make_latest':'true'},'PATCH')
assert not public['draft'] and not public['prerelease'] and public['published_at']
assert api('git/ref/tags/'+TAG)['object']['sha']==SOURCE and api('git/ref/heads/main')['object']['sha']==SOURCE
# Read-only confirmation may be retried; the write and uploads are not repeated.
for attempt in range(4):
    latest=api('releases/latest')
    if latest['id']==public['id'] and latest['tag_name']==TAG:break
    time.sleep(2)
else:raise RuntimeError('Published release is not yet confirmed as Latest; inspect without republishing')
receipt={'release_url':public['html_url'],'source_commit':SOURCE,'tag':TAG,'main_updated':True,'readback_verified':True,'stable_latest_verified':True,
    'assets':[{'name':p.name,'size':p.stat().st_size,'sha256':digest(p)} for p in files]}
Path('aiTemp/monitor-release-receipt.json').write_text(json.dumps(receipt,indent=2)+'\n')
