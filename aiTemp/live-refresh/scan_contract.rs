...
fn refresh_contract_baseline_rejects_oversized_input_without_saving_partial_task() {
    ...
    assert!(
        rejected.is_err(),
        "BASELINE_SCAN_UNBOUNDED: oversized files must not be read into a baseline and accepted"
    );
    assert!(
        harness.current_task().unwrap().is_none(),
        "A rejected scan must not create an apparently valid task"
    );
    ...
    std::fs::rename(
        root.join("large-model.onnx"),
        base.join("Trash/large-model.onnx"),
    )
    .unwrap();
    ...
    ignored.set_len(32 * 1024 * 1024 + 1).unwrap();
    drop(ignored);
    ...
    assert!(
        harness.check_baseline(&task.id).is_err(),
        "Incomplete or stale baselines must never authorize writes"
    );
