...
REPO='p90-lover/coding-tools-mcp';BASE='22638b41ba67713502119ad6f54b455be8104112'
...
assert '1 passed; 0 failed' in (root/'windows/redirect-tests.txt').read_text(encoding='utf-8')
...
(assets/'provenance.json').write_text(json.dumps({'version':VERSION,'source_commit':SOURCE,'workflow_run':int(RUN),'preserved_main_commit':BASE,'windows':proof,'browser':ui,'client_connection_verified':False},indent=2)+'\n')
...
# New publication only. Preserve the concurrent main OAuth fix and abort on any further changes.
...
release=api('releases',{'tag_name':TAG,'target_commitish':SOURCE,'name':'Coding Tools MCP '+TAG+' — Tool exposure and callback repair','body':notes,'draft':True,'prerelease':True,'make_latest':'false'})
...
receipt={'url':public['html_url'],'source_commit':SOURCE,'tag':TAG,'main_updated':True,'preserved_main_commit':BASE,'assets_retrieved_and_verified':True,'assets':[{'name':p.name,'size':p.stat().st_size,'sha256':digest(p)} for p in files]}
