"""Publish verified immutable installers; no rebuild, deletion, or model calls."""
...
for step in ('Native isolation with positive controls','Real Windows background observation and input','Build and verify actual installer resources'):
...
windows=[p for p in evidence.iterdir() if 'windows-x64' in p.name];require(len(windows)==1,'Windows evidence must be unique');we=windows[0]
isolation=json.loads((we/'sandbox-proof.json').read_text())
...
for folder in evidence.iterdir():
    ...
    require(json.loads((folder/'proof.json').read_text())['source_commit']==source,'Evidence source mismatch')
...
origin=json.loads((we/'sandbox-build-origin.json').read_text())
require(origin['source_commit']=='918d56936340055d3482ece655e3952c2227565c' and origin['workflow_run']==34206606095,'Unexpected helper compilation origin')
...
require(api('git/ref/heads/release/remembered-control-final')['object']['sha']==source,'Release source branch advanced')
...
old_en='with local-only one-time OS preparation, read-only workspace/platform access and restricted networking.'
new_en='with local-only one-time OS preparation, blocked workspace writes and restricted networking. Reads follow the dedicated sandbox account Windows ACLs, not a path whitelist: shared readable files outside the workspace may remain readable.'
old_zh='首次系統設定須本機同意；僅唯讀工作區／平台存取及限制網絡，'
new_zh='首次系統設定須本機同意；阻止工作區寫入並限制網絡。讀取依據沙箱帳戶的 Windows ACL，而非路徑白名單：工作區外共用可讀檔案仍可能可讀；'
require(notes.count(old_en)==1 and notes.count(old_zh)==1,'Release scope correction no longer matches source notes')
notes=notes.replace(old_en,new_en).replace(old_zh,new_zh)
notes+='\n\nValidation: exact-source Windows and macOS builds passed. Native positive/negative controls verified permitted reads, denied workspace write handles, denied ACL-protected private-file reads (shared files outside the workspace remain readable), and denied direct loopback networking. The real Windows background/UIA/input/RAM-image fixture passed. An old sandbox report was retained byte-for-byte in protected Trash. Helper build provenance and all binary hashes were checked before reuse; native tests were rerun, not waived.\n\n驗證：相同原始碼的 Windows／macOS 建置通過；原生正反向測試確認可讀取、拒絕工作區寫入、拒絕 ACL 保護的私人檔案讀取（範圍外共用可讀檔案仍可讀）及拒絕直接迴路網絡。真實背景視窗／UIA／輸入／記憶體圖片測試通過；舊沙箱報告完整保留於受保護 Trash。輔助程式來源及雜湊已核對，原生測試已重新執行，沒有略過。\n'
...
require(api('git/ref/tags/'+tag)['object']['sha']==source and api('git/ref/heads/release/remembered-control-final')['object']['sha']==source,'Release source moved')
