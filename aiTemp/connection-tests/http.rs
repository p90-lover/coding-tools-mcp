...
use crate::tools::{ToolContext, live_policy::commit_updates};
...
impl Fixture {
    async fn new(oauth_mode: bool) -> Self {
        let root = std::env::current_dir().unwrap().join("aiTemp/connection-http")
        ...
        let jwt = |exp: u64| jsonwebtoken::encode(&jsonwebtoken::Header::default(),
            &json!({"iss":format!("urn:coding-tools-mcp:{wid}"),"aud":wid,"wid":wid,
                "iat":now - 3600,"exp":exp,"scope":"mcp"}),
            &jsonwebtoken::EncodingKey::from_secret(key.as_bytes())).unwrap();
        let token = if oauth_mode { jwt(now + 3600) } else { "isolated-http-test-token-not-real".into() };
        let expired = jwt(now - 60);
        let oauth = oauth_mode.then(|| Arc::new(OAuthRuntime::new(wid, "fixture-client".into(), None,
            "fixture-password-not-real".into(), key.into())));
        let state = ListenerState {
            mcp: context.clone(), auth: context.auth.clone(),
            ...
            bind_port: port, configured_public_url: "https://repair.example".into(),
            bearer_token: (!oauth_mode).then(|| token.clone()), oauth, oauth_client_secret: None,
        };
        let (stop, shutdown) = oneshot::channel();
        let worker = tokio::spawn(async move { serve(socket, port, state, shutdown).await.unwrap(); });
        ...
        Self { endpoint: format!("http://127.0.0.1:{port}/mcp"), root, token, expired, client, context, _stop: stop, worker }
    }
    async fn rpc(&self, body: Value) -> reqwest::Response {
        self.client.post(&self.endpoint).bearer_auth(&self.token)
            .header("Accept", "application/json, text/event-stream").json(&body).send().await.unwrap()
    }
}
...
async fn connection_repair_oauth_challenge_and_token_recovery() {
    ...
    for token in [None, Some(f.expired.as_str()), Some("invalid-token")] {
        let mut r = f.client.post(&f.endpoint).header("x-forwarded-host", "attacker.invalid")
            .json(&json!({"jsonrpc":"2.0","id":1,"method":"tools/list"}));
        if let Some(token) = token { r = r.bearer_auth(token); }
    ...
    let tools: Value = f.rpc(json!({"jsonrpc":"2.0","id":2,"method":"tools/list"})).await.json().await.unwrap();
    assert!(!tools["result"]["tools"].as_array().unwrap().is_empty());
    let origin = f.endpoint.trim_end_matches("/mcp");
    let mut docs = Vec::new();
    for path in ["/.well-known/oauth-protected-resource", "/.well-known/oauth-protected-resource/mcp"] {
        docs.push(f.client.get(format!("{origin}{path}")).send().await.unwrap().json::<Value>().await.unwrap());
    }
    assert_eq!(docs[0], docs[1]);
    assert_eq!(docs[0]["authorization_servers"], json!(["https://repair.example"]));
    let log = std::fs::read_to_string(f.root.join("transport-logs/mcp-requests.log")).unwrap();
    assert!(log.contains("authentication_rejected status=401") && log.contains("catalog_served tools_count="));
...
async fn connection_repair_handshake_notifications_catalog_and_live_policy() {
    ...
    let ack = f.rpc(json!({"jsonrpc":"2.0","method":"notifications/initialized"})).await;
    assert_eq!(ack.status().as_u16(), 202);
    assert!(ack.bytes().await.unwrap().is_empty());
    let before: Value = f.rpc(json!({"jsonrpc":"2.0","id":3,"method":"tools/list"})).await.json().await.unwrap();
    assert!(before["result"]["tools"].as_array().unwrap().iter().any(|t| t["name"] == "history_session_bootstrap"));
    ...
    let after: Value = f.rpc(json!({"jsonrpc":"2.0","id":4,"method":"tools/list"})).await.json().await.unwrap();
    ...
    let mut idless = patch; idless.as_object_mut().unwrap().remove("id");
    assert_eq!(f.rpc(idless).await.status().as_u16(), 400);
    assert!(!f.root.join("rejected.txt").exists());
    assert!(!f.worker.is_finished(), "same listener; no relink or restart");
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn connection_repair_sse_get_and_legacy_probe_are_distinct() {
    let f = Fixture::new(true).await;
    let denied = f.client.get(&f.endpoint).header("Accept", "text/event-stream").send().await.unwrap();
    assert_eq!(denied.status().as_u16(), 401);
    assert!(denied.headers().contains_key("www-authenticate"));
    for accept in ["text/event-stream", "application/json, text/event-stream; q=1"] {
        let stream = f.client.get(&f.endpoint).header("Accept", accept).bearer_auth(&f.token).send().await.unwrap();
        assert_eq!(stream.status().as_u16(), 405);
        assert_eq!(stream.headers()["allow"], "POST");
    }
    let discovery = f.client.get(&f.endpoint).header("Accept", "application/json").send().await.unwrap();
    ...
    assert!(body.get("tools").is_none(), "public health does not expose the tool catalog");
}
