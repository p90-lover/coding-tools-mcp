"""Repair the reproduced cmd parser failure; preserve isolation checks and originals."""
...
if 'cmd_parser_verified' not in text:
    insert=r'''# Keep cmd grammar coverage in the existing native filesystem boundary group.
...
if 'source space.txt' not in text:
    ...
    part=once(part,r'"type %MCP_SANDBOX_INPUT%\\source space.txt"',r'"type \"%MCP_SANDBOX_INPUT%\\source space.txt\""')
...
print('Command-parser patch applied; AppContainer token/SID, grants and revocation unchanged.')
