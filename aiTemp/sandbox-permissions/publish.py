"""Read back all assets before publishing and non-forced main integration."""
from pathlib import Path
import hashlib,json,os,shutil,subprocess,zipfile
REPO='p90-lover/coding-tools-mcp';BASE='35b13594ddb3ba70011b2c47cec840e6a0dba3e1'
BRANCH='fix/sandbox-permissions-0.4.4-rc.1';VERSION='0.4.4-rc.1';TAG='v'+VERSION
...
subprocess.run(['git','diff','--exit-code',BASE,SOURCE,'--','src-tauri/src/auth','src-tauri/src/mcp','aiTemp/origin-repair'],check=True)
...
root=Path('aiTemp/snapshot-publication');root.mkdir(parents=True,exist_ok=False)
download('snapshot-installer-'+RUN,root/'installer');download('snapshot-evidence-'+RUN,root/'evidence')
...
assert proof['snapshot_contract_groups_passed']==3 and proof['snapshot_native_groups_passed']==3
assert proof['snapshot_helper_embedded'] and proof['snapshot_local_consent_required'] and not proof['snapshot_host_fallback']
assert not proof['snapshot_direct_workspace_write'] and proof['snapshot_network_capabilities']==0
assert not proof['official_codex_sandbox_implementation'] and not proof['live_user_snapshot_verified']
assert proof['oauth_regressions_passed']==5 and proof['browser_origin_cases_passed']==3
assert proof['null_origin_still_denied'] and not proof['codex_executable_invoked'] and proof['model_requests']==0
assert proof['binary_verification']['all_other_bytes_identical'] and not proof['publisher_signed']
assert proof['catalog_counts']=={'core':57,'advanced':70} and proof['read_only_catalog_count']==41
native=json.loads((root/'evidence/native-proof.json').read_text());assert native==proof['snapshot_native_evidence']
assert native['helper_sha256']==proof['snapshot_helper_sha256'] and native['passed_groups']==3
...
for name,count in [('snapshot-contract.txt',3),('registry.txt',1),('oauth-tests.txt',5),('catalog-tests.txt',2)]:
...
provenance={'version':VERSION,'source_commit':SOURCE,'main_commit':SOURCE,'workflow_run':int(RUN),
    'preserved_main':BASE,'windows':proof,'snapshot_native':native,'browser':browser,
    'extension_source':'b0d4db553227fa3c7300050b8edff6e2eb9afee4','extension_version':'0.0.7',
    'live_user_connection_verified':False,'live_user_snapshot_verified':False,'model_requests':0}
...
notes+=f'\n\nExact source / 原始碼: `{SOURCE}`\nBuild / 建置: https://github.com/{REPO}/actions/runs/{RUN}\nRead-only catalogue / 唯讀工具目錄: 41 tools (sandbox_exec excluded / 不包括 sandbox_exec).\n'
release=api('releases',{'tag_name':TAG,'target_commitish':SOURCE,'name':'Coding Tools MCP '+TAG+' — offline snapshot sandbox and permissions','body':notes,'draft':True,'prerelease':True,'make_latest':'false'})
Path('aiTemp/snapshot-release-draft.json').write_text(json.dumps({'release_id':release['id'],'source':SOURCE}))
...
Path('aiTemp/snapshot-release-receipt.json').write_text(json.dumps(receipt,indent=2)+'\n');print(json.dumps(receipt,indent=2))
