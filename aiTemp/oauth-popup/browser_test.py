"""Replay production OAuth responses in Chromium, including actual redirect hops.
No external connection, real account, screenshot, or model request is permitted.
"""
...
with sync_playwright() as p:
    browser = p.chromium.launch(
        ...
        args=['--no-sandbox', '--proxy-server=http://127.0.0.1:9'],
    )
    for case in ['old_policy', 'fixed_policy', 'wrong_destination']:
        ...
        def route_handler(route):
            ...
            if request.url == 'https://chatgpt.com/test-initiator':
                ...
                route.fulfill(status=200, content_type='text/html', body=f'<button id="connect">Connect</button><script>document.getElementById("connect").onclick=()=>window.open({url},"oauth","popup=yes,width=560,height=720");</script>')
                return
            if parts.netloc == 'old-popup.example' and parts.path == '/oauth/authorize':
                if request.method == 'GET':
                    assert request.url == fixture['authorize_url']
                    route.fulfill(status=200, headers={'Content-Type': 'text/html; charset=utf-8', 'Content-Security-Policy': csp, 'Set-Cookie': fixture['set_cookie'], 'Cache-Control': 'no-store'}, body=fixture['html'])
                    return
                if request.method == 'POST':
                    ...
                    route.fulfill(status=fixture['redirect_status'], headers={'Location': callback, 'Cache-Control': 'no-store'}, body='')
                    return
        ...
        # Playwright's high-level route skips redirected URLs. CDP sees the actual
        # follow-up request, after Chromium applies CSP. It does not force navigation,
        # change the 303 response, or bypass the negative form-action cases.
        ...
        def on_redirect(event):
            ...
            if request['url'] != callback:
                ...
                return
            has_body = bool(request.get('hasPostData') or request.get('postData'))
            calls.append({'method': request['method'], 'target': 'callback', 'has_post_body': has_body})
            assert request['method'] == 'GET' and not has_body
            assert 'password' not in parse_qs(urlsplit(request['url']).query)
        ...
        try:
            ...
            if case == 'fixed_policy':
                ...
                assert popup.url == callback
                assert any(c['target'] == 'callback' and c['method'] == 'GET' and not c['has_post_body'] for c in calls)
                assert not errors, errors
            else:
                for _ in range(30):
                    if any('form-action' in error for error in errors):
                        break
                    popup.wait_for_timeout(100)
                assert any('form-action' in error for error in errors), errors
                assert not any(c['target'] == 'callback' for c in calls), calls
            assert any(c['target'] == 'authorization_form' for c in calls)
            results.append({'case': case, 'callback_reached': any(c['target'] == 'callback' for c in calls), 'requests': calls, 'policy_violation': any('form-action' in error for error in errors)})
        except Exception:
            print(json.dumps({'failed_case': case, 'url': popup.url, 'requests': calls, 'console_errors': errors}), flush=True)
            raise
        finally:
            context.close()
    version = browser.version
    browser.close()
proof = {'source': fixture['source'], 'browser': version, 'passed': True, 'cases': results, 'live_chatgpt_account': False, 'transport': fixture['transport'], 'redirect_interception': 'CDP Fetch; normal browser CSP retained', 'external_network_blocked': True, 'model_requests': 0, 'screenshots_written': 0}
