...
const releaseVersion = process.env.RELEASE_VERSION ?? '0.7.0-rc.11';
...
const childEnv = {
  ...
  VALIDATION_WORKFLOW: '.github/workflows/codex-router-multiprovider-release-rc11.yml',
...
async function main() {
  ...
  assert(releaseVersion === '0.7.0-rc.11', `RELEASE_VERSION_MISMATCH:${releaseVersion}`);
  assert(releaseTag === 'v0.7.0-rc.11', `RELEASE_TAG_MISMATCH:${releaseTag}`);
  assert(repository, 'GITHUB_REPOSITORY_MISSING');
  assert(
    branch === 'main' || branch === 'release/codex-router-multiprovider-0.7.0-rc.8',
    `RELEASE_BRANCH_MISMATCH:${branch}`,
  );
  ...
  run('node', ['--test',
    ...
    'desktop-electron/tests/rc10-release-identity.test.cjs',
    ...
    'desktop-electron/tests/rc11-five-stack-original-parity.test.cjs',
    'desktop-electron/tests/managed-bootstrap.test.cjs',
    'desktop-electron/tests/managed-bootstrap-ipc-contract.test.cjs',
    'desktop-electron/tests/managed-bootstrap-renderer-contract.test.cjs',
    'desktop-electron/tests/upstream-tool-runtime.test.cjs',
    'desktop-electron/tests/rc9-managed-cpa-runtime.test.cjs',
  ], { label: 'focused five-stack original parity, bootstrap, and rc.11 identity contracts' });
  ...
  for (const file of [
    ...
    'desktop-electron/electron/managed-bootstrap.cjs',
  ...
  const provenance = {
    ...
    validation_run: { id: validationRunId, url: validationRunUrl, workflow: '.github/workflows/codex-router-multiprovider-release-rc11.yml' },
  ...
  run('node', ['scripts/release/publish-v0.6.0-rc.1.mjs', '--repository', repository, '--tag', releaseTag, '--source', sourceSha, '--assets', assets, '--notes', 'docs/releases/v0.7.0-rc.11.md', '--json-out', path.join(evidence, 'publication.json')], { label: 'publish and read back rc.11 prerelease' });
  ...
  process.stdout.write(`\nRC11_WINDOWS_RELEASE_PUBLISHED ${releaseTag} ${sourceSha} ${installerSha}\n`);
}
