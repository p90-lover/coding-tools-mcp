...
async fn check(root: PathBuf) {
    ...
    let state = ListenerState {
        ...
        bearer_token: Some("synthetic-recovery-bearer".into()),
        oauth: None,
    ...
    let response = client
        .post(&url)
        .bearer_auth("synthetic-recovery-bearer")
        .json(&request)
        .send()
        .await
        .unwrap();
    assert_eq!(response.status(), 200);
    let operation = response.headers()["x-mcp-operation-id"]
        .to_str()
        .unwrap()
        .to_string();
    ...
    assert_eq!(
        original["result"]["_meta"]["coding-tools-mcp/operation"]["operation_id"],
        operation
    );
    ...
    for _ in 0..16 {
        slots.push(crate::auth::http_security::acquire_tool_worker().unwrap());
    }
    let polled = client
        .post(&url)
        .bearer_auth("synthetic-recovery-bearer")
    ...
    assert_eq!(
        ctx.operations
            .query(&json!({}), 0, &["server_info"])
            .unwrap()["operations"]
            .as_array()
            .unwrap()
            .len(),
        1,
        "Lookup/capacity rejection must not dispatch another tracked operation"
    );
    assert!(
        !root.join("workspace/docs/history-session").exists(),
        "Read-only lookup must not create history"
    );
    let other =
        crate::tools::ToolContext::for_test(root.join("workspace"), root.join("other-harness"))
            .unwrap();
    assert_eq!(
        other
            .operations
            .query(&json!({"operation_id":operation}), 0, &["server_info"])
            .unwrap()["found"],
        false,
        "Same directory is not same listener authority"
    );
    let definition = crate::tools::registry::list_tools()
        .into_iter()
        .find(|d| d["name"] == "mcp_operation_status")
        .unwrap();
    ...
    println!(
        "RECOVERY_SCHEMA_EVIDENCE {}",
        json!({"definition":definition,"success":structured})
    );
    drop(slots);
    let _ = shutdown.send(());
    server.await.unwrap();
