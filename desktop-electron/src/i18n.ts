...
const en = {
  ...
  hideTab: "Close tab",
  newTab: "New tab",
  browserTabLimit: "Up to five additional ChatGPT tabs, shared by browsing and Codex tasks.",
  ...
  connectorMigrationNotice: "Upgrading from Codex Native? Leave the old connector untouched and use the current connector name shown below. Verify runtime rejects a legacy-only setup.",
  ...
  autoCreateConnector: "Create connector from saved setup",
  connectorCreated: "Connector created. Check its permissions; runtime verification will run when Codex is ready.",
  connectorAlreadyExists: "A connector with this name already exists. Check its chat availability and permissions before verification.",
  ...
  autoConnectExistingMcp: "Auto-connect existing MCP harness",
  autoConnectExistingMcpBody: "After Codex reaches the ready bridge, verify and connect the saved MCP harness. This never creates a connector, tunnel, or key.",
  ...
  moreTools: "More",
  providers: "Providers",
  providerOAuth: "Accounts",
  apiModels: "API Models",
  structuredOrchestrator: "Structured Orchestrator",
  workspaceAuth: "Workspace Authentication",
  workspaceAuthBody: "Edit MCP and Actions authentication for the selected workspace. Secret values stay in local storage.",
  nativeCodex: "Native Codex",
  nativeCodexBody: "Connect a separately signed-in local Codex App Server to a running authenticated workspace MCP listener. ChatGPT can then use codex_agent_control without an active Codex task.",
  nativeCodexExecutable: "Native codex.exe path",
  nativeCodexHome: "Dedicated Codex home",
  nativeCodexModel: "Model ID",
  nativeCodexPermissionProfile: "Native permission profile",
  nativeCodexRequestLimit: "Model requests (0 = no local limit)",
  nativeCodexLifetime: "Consent seconds (0 = until Stop)",
  nativeCodexModelUse: "Allow model use for this connection",
  nativeCodexCommandUse: "Allow separate read-only commands",
  nativeCodexReadOnly: "Native agent turns currently use read-only permissions and decline approval prompts. Stop locally to revoke this connection.",
  nativeCodexWorkspaceNote: "The :workspace profile requests writes inside this workspace and an elevated Windows sandbox. One existing outside file may be approved locally for the current turn; other extra permissions are denied. OS enforcement is not independently verified here.",
  nativeCodexConnect: "Connect App Server",
  nativeCodexStop: "Stop and disconnect",
  nativeCodexConnected: "Connected",
  nativeCodexDisconnected: "Not connected",
  nativeCodexLimits: "Use 0–20 requests and 0 or 30–900 seconds.",
  workspaceService: "Service",
  workspaceAuthType: "Authentication type",
  workspaceClientId: "OAuth client ID",
  workspaceRedirectUris: "OAuth callback URLs, one per line",
  workspaceScopes: "OAuth scopes",
  workspaceSharedSecrets: "Use shared secrets",
  workspaceSaveAuth: "Save authentication",
  workspaceAuthSaved: "Saved. Restart the listener to apply this setting.",
  workspaceAuthCancelled: "No changes saved.",
  workspaceAuthSecretNote: "Secrets stay in local storage. Copying one may generate it if missing and requires local confirmation.",
  workspaceCopyCredential: "Copy credential",
  workspaceCredentialCopied: "Credential copied to clipboard.",
  workspaceListenerStatus: "Local listener",
  workspaceStart: "Start",
  workspaceStop: "Stop",
  workspaceRestart: "Restart",
  workspaceListenerTunnelNote: "This controls the local listener only. Saved tunnel settings are unchanged.",
  integrations: "Integrations",
  paseoOrchestrator: "Paseo Orchestrator",
  annealTasks: "Anneal Tasks",
  networkProxy: "Network Proxy",
  liveMcpTools: "Workspace",
  liveMcpToolsBody: "Review each local workspace path and the permissions applied to MCP tools.",
  workspacePath: "Path",
  workspaceReadAccess: "Read",
  workspaceReadScope: "Workspace, linked projects, and explicit absolute paths",
  workspaceWriteAccess: "Edit",
  workspaceWriteScope: "Workspace and writable linked projects",
  workspaceBlocked: "Blocked",
  workspaceUnknown: "Unknown",
  workspacePermissionMode: "Permission mode",
  workspaceApprovalMode: "Approval mode",
  workspaceToolProfile: "Tool catalog",
  workspaceEditPolicy: "Edit permissions",
  workspaceSavePolicy: "Save permissions",
  workspaceCapture: "Allow screen capture",
  workspacePolicyRestart: "Restart the MCP listener after saving. Computer input also needs separate local pairing.",
  workspaceOutsideScope: "Ask for local approval before editing an outside project. File tools block writes until it is an approved writable linked project.",
  inProcessApps: "In-process module handlers",
  inProcessAppsBody: "List registered modules and call them through codingTools.apps.call / invoke. Handlers stay in-process; they do not open dedicated listen ports or extra app windows. ProxyBridge seed is http://127.0.0.1:17891.",
  inProcessAppsTransport: "In-process",
  inProcessAppsCall: "apps.call",
  inProcessAppsInvoke: "apps.invoke",
  inProcessAppsNoModules: "No in-process module handlers are registered yet.",
  selectModule: "Module",
...
const zh: Record<keyof typeof en, string> = {
  ...
  hideTab: "关闭标签页",
  newTab: "新建标签页",
  browserTabLimit: "最多可打开五个额外的 ChatGPT 标签页，浏览和 Codex 任务共用此上限。",
  ...
  connectorMigrationNotice: "从 Codex Native 升级？请保留旧连接器不变，并使用下方显示的当前连接器名称。仅有旧连接器时，运行时验证会明确失败。",
  ...
  autoCreateConnector: "使用已保存的配置创建连接器",
  connectorCreated: "连接器已创建。请检查权限；Codex 准备就绪后会自动验证运行时。",
  connectorAlreadyExists: "已存在同名连接器。验证前请检查它在当前聊天中的可用性和权限。",
  connectorName: "连接器名称",
  verifyRuntime: "验证运行时",
  ...
  autoConnectExistingMcp: "自动连接现有 MCP Harness",
  autoConnectExistingMcpBody: "Codex 连接到就绪的 Bridge 后，验证并连接已保存的 MCP Harness。此选项绝不会创建连接器、Tunnel 或密钥。",
  ...
  providerOAuth: "账户",
  apiModels: "API 模型",
  structuredOrchestrator: "结构化编排器",
  workspaceAuth: "工作区认证",
  workspaceAuthBody: "编辑所选工作区的 MCP 和 Actions 认证。密钥保留在本地存储中。",
  nativeCodex: "原生 Codex",
  nativeCodexBody: "将单独登录的本地 Codex App Server 连接到已启动且经过认证的工作区 MCP 监听器。ChatGPT 无需活动中的 Codex 任务即可使用 codex_agent_control。",
  nativeCodexExecutable: "原生 codex.exe 路径",
  nativeCodexHome: "独立的 Codex 主目录",
  nativeCodexModel: "模型 ID",
  nativeCodexPermissionProfile: "原生权限配置",
  nativeCodexRequestLimit: "模型请求上限（0 = 本地不限）",
  nativeCodexLifetime: "授权秒数（0 = 直到停止）",
  nativeCodexModelUse: "允许此连接使用模型",
  nativeCodexCommandUse: "允许单独执行只读命令",
  nativeCodexReadOnly: "原生代理回合目前使用只读权限，并拒绝批准请求。可在本地停止以撤销连接。",
  nativeCodexWorkspaceNote: ":workspace 会请求工作区内写入及 Windows 提权沙箱。工作区外的单个现有文件可在本地批准，仅限当前回合；其他额外权限会被拒绝。本应用未独立验证操作系统沙箱。",
  nativeCodexConnect: "连接 App Server",
  nativeCodexStop: "停止并断开",
  nativeCodexConnected: "已连接",
  nativeCodexDisconnected: "未连接",
  nativeCodexLimits: "请求上限为 0–20；授权时长为 0 或 30–900 秒。",
  workspaceService: "服务",
  workspaceAuthType: "认证类型",
  workspaceClientId: "OAuth 客户端 ID",
  workspaceRedirectUris: "OAuth 回调 URL，每行一个",
  workspaceScopes: "OAuth 权限范围",
  workspaceSharedSecrets: "使用共享密钥",
  workspaceSaveAuth: "保存认证",
  workspaceAuthSaved: "已保存。重启监听器后生效。",
  workspaceAuthCancelled: "未保存更改。",
  workspaceAuthSecretNote: "密钥保留在本地存储中。复制时若缺失会生成密钥，并需要本地确认。",
  workspaceCopyCredential: "复制密钥",
  workspaceCredentialCopied: "密钥已复制到剪贴板。",
  workspaceListenerStatus: "本地监听器",
  workspaceStart: "启动",
  workspaceStop: "停止",
  workspaceRestart: "重启",
  workspaceListenerTunnelNote: "这里只控制本地监听器，不会更改已保存的隧道设置。",
  integrations: "集成服务",
  paseoOrchestrator: "Paseo 协调器",
  annealTasks: "Anneal 任务",
  networkProxy: "网络代理",
  liveMcpTools: "工作区",
  liveMcpToolsBody: "查看本地工作区路径及 MCP 工具的实际权限。",
  workspacePath: "路径",
  workspaceReadAccess: "读取",
  workspaceReadScope: "工作区、已关联项目和显式绝对路径",
  workspaceWriteAccess: "编辑",
  workspaceWriteScope: "工作区和可写的已关联项目",
  workspaceBlocked: "已阻止",
  workspaceUnknown: "未知",
  workspacePermissionMode: "权限模式",
  workspaceApprovalMode: "批准模式",
  workspaceToolProfile: "工具目录",
  workspaceEditPolicy: "编辑权限",
  workspaceSavePolicy: "保存权限",
  workspaceCapture: "允许屏幕捕获",
  workspacePolicyRestart: "保存后重启 MCP 监听器。电脑输入还需要单独在本地配对。",
  workspaceOutsideScope: "编辑外部项目前须征得本地批准。成为已批准的可写关联项目前，文件工具会阻止写入。",
  inProcessApps: "进程内模块处理程序",
  inProcessAppsBody: "列出已注册模块，并通过 codingTools.apps.call / invoke 调用。处理程序在进程内运行，不会开放专用监听端口或额外应用窗口。ProxyBridge 种子为 http://127.0.0.1:17891。",
  inProcessAppsTransport: "进程内",
  inProcessAppsCall: "apps.call",
  inProcessAppsInvoke: "apps.invoke",
  inProcessAppsNoModules: "尚未注册进程内模块处理程序。",
  selectModule: "模块",
  selectOperation: "操作",
  refreshTools: "刷新",
  runTool: "运行工具",
...
const ja: Record<keyof typeof en, string> = {
  ...
  hideTab: "タブを閉じる",
  newTab: "新しいタブ",
  browserTabLimit: "追加の ChatGPT タブは最大 5 件です。閲覧タブと Codex タスクでこの上限を共有します。",
  ...
  connectorMigrationNotice: "Codex Native からアップグレードする場合は、古いコネクタを変更せず、下に表示される現在のコネクタ名を使用してください。古いコネクタしかない場合、ランタイム検証は失敗します。",
  ...
  autoCreateConnector: "保存済み設定からコネクタを作成",
  connectorCreated: "コネクタを作成しました。権限を確認してください。Codex の準備後にランタイムを自動検証します。",
  connectorAlreadyExists: "同じ名前のコネクタが存在します。検証前に現在のチャットでの利用可否と権限を確認してください。",
  connectorName: "コネクタ名",
  ...
  autoConnectExistingMcp: "既存の MCP ハーネスに自動接続",
  autoConnectExistingMcpBody: "Codex が準備完了したブリッジに接続した後、保存済みの MCP ハーネスを検証して接続します。コネクタ、トンネル、キーは作成しません。",
  ...
  providerOAuth: "アカウント",
  apiModels: "API モデル",
  structuredOrchestrator: "構造化オーケストレーター",
  workspaceAuth: "ワークスペース認証",
  workspaceAuthBody: "選択したワークスペースの MCP と Actions の認証を編集します。秘密情報はローカルに保持されます。",
  nativeCodex: "ネイティブ Codex",
  nativeCodexBody: "別途サインインしたローカル Codex App Server を、認証済みの稼働中ワークスペース MCP リスナーに接続します。ChatGPT はアクティブな Codex タスクがなくても codex_agent_control を使えます。",
  nativeCodexExecutable: "ネイティブ codex.exe のパス",
  nativeCodexHome: "専用 Codex ホーム",
  nativeCodexModel: "モデル ID",
  nativeCodexPermissionProfile: "ネイティブ権限プロファイル",
  nativeCodexRequestLimit: "モデル要求上限（0 = ローカル上限なし）",
  nativeCodexLifetime: "同意期間（秒、0 = 停止まで）",
  nativeCodexModelUse: "この接続でモデル利用を許可",
  nativeCodexCommandUse: "個別の読み取り専用コマンドを許可",
  nativeCodexReadOnly: "ネイティブのエージェントターンは現在読み取り専用で、承認要求を拒否します。ローカルの停止ボタンで接続を取り消せます。",
  nativeCodexWorkspaceNote: ":workspace はこのワークスペース内の書き込みと Windows の elevated サンドボックスを要求します。外部の既存ファイル 1 件は現在のターンに限りローカルで承認できます。その他の追加権限は拒否します。OS サンドボックスの強制は独立検証していません。",
  nativeCodexConnect: "App Server に接続",
  nativeCodexStop: "停止して切断",
  nativeCodexConnected: "接続中",
  nativeCodexDisconnected: "未接続",
  nativeCodexLimits: "要求数は 0～20、同意期間は 0 または 30～900 秒にしてください。",
  workspaceService: "サービス",
  workspaceAuthType: "認証方式",
  workspaceClientId: "OAuth クライアント ID",
  workspaceRedirectUris: "OAuth コールバック URL（1 行に 1 件）",
  workspaceScopes: "OAuth スコープ",
  workspaceSharedSecrets: "共有シークレットを使用",
  workspaceSaveAuth: "認証を保存",
  workspaceAuthSaved: "保存しました。適用するにはリスナーを再起動してください。",
  workspaceAuthCancelled: "変更は保存されませんでした。",
  workspaceAuthSecretNote: "秘密情報はローカルに保持されます。コピー時に未作成なら生成され、ローカル確認が必要です。",
  workspaceCopyCredential: "資格情報をコピー",
  workspaceCredentialCopied: "資格情報をクリップボードにコピーしました。",
  workspaceListenerStatus: "ローカルリスナー",
  workspaceStart: "開始",
  workspaceStop: "停止",
  workspaceRestart: "再起動",
  workspaceListenerTunnelNote: "ローカルリスナーのみを操作します。保存済みのトンネル設定は変更されません。",
  integrations: "統合サービス",
  paseoOrchestrator: "Paseo オーケストレーター",
  annealTasks: "Anneal タスク",
  networkProxy: "ネットワークプロキシ",
  liveMcpTools: "ワークスペース",
  liveMcpToolsBody: "ローカルワークスペースのパスと MCP ツールの権限を確認します。",
  workspacePath: "パス",
  workspaceReadAccess: "読み取り",
  workspaceReadScope: "ワークスペース、リンク済みプロジェクト、明示的な絶対パス",
  workspaceWriteAccess: "編集",
  workspaceWriteScope: "ワークスペースと書き込み可能なリンク済みプロジェクト",
  workspaceBlocked: "ブロック",
  workspaceUnknown: "不明",
  workspacePermissionMode: "権限モード",
  workspaceApprovalMode: "承認モード",
  workspaceToolProfile: "ツール一覧",
  workspaceEditPolicy: "権限を編集",
  workspaceSavePolicy: "権限を保存",
  workspaceCapture: "画面キャプチャを許可",
  workspacePolicyRestart: "保存後に MCP リスナーを再起動してください。PC 操作には別途ローカルでのペアリングが必要です。",
  workspaceOutsideScope: "外部プロジェクトを編集する前にローカル承認を求めてください。書き込み可能なリンク先として承認されるまで、ファイルツールは書き込みを拒否します。",
  inProcessApps: "プロセス内モジュールハンドラー",
  inProcessAppsBody: "登録済みモジュールを一覧し、codingTools.apps.call / invoke で呼び出します。ハンドラーはプロセス内で動作し、専用リスンポートや別ウィンドウは開きません。ProxyBridge シードは http://127.0.0.1:17891 です。",
  inProcessAppsTransport: "プロセス内",
  inProcessAppsCall: "apps.call",
  inProcessAppsInvoke: "apps.invoke",
  inProcessAppsNoModules: "プロセス内モジュールハンドラーはまだ登録されていません。",
  selectModule: "モジュール",
  selectOperation: "操作",
  refreshTools: "更新",
  runTool: "ツールを実行",
