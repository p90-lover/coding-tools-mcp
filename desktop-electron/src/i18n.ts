...
const en = {
  ...
  instantMcpToolsHandlerStub: "apps_list is callable now. Bot GG still owns the instant-mcp-tools app-handler (inspect / list-tools / run-tool). This UI does not open a dedicated listen port.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  instantMcpToolsHandlerStub: "现在可以调用 apps_list。instant-mcp-tools 的 app-handler 仍由 Bot GG 负责（inspect / list-tools / run-tool）。此界面不会开放专用监听端口。",
...
const ja: Record<keyof typeof en, string> = {
  ...
  instantMcpToolsHandlerStub: "apps_list は既に呼び出せます。instant-mcp-tools の app-handler は Bot GG 担当（inspect / list-tools / run-tool）です。この UI は専用リスンポートを開きません。",
