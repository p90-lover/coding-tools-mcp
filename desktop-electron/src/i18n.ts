...
const en = {
  ...
  liveMcpToolsMoved: "Live MCP tools moved to Apps. Open Instant MCP Tools to pick a workspace, tool, and params, then run the tool.",
  instantMcpTools: "Instant MCP Tools",
  instantMcpToolsHostBody: "Workspace, tool, params, and Run tool live here. Handlers stay in-process through codingTools.apps; this app does not open a listen port or use Start.",
  instantMcpToolsHandlerStub: "apps_list is callable now. Bot GG still owns the instant-mcp-tools app-handler (inspect / listTools / runTool / listWorkspaces; kebab aliases list-tools / run-tool). This UI does not open a dedicated listen port.",
  instantMcpToolsConnected: "Connected",
  instantMcpToolsInProcessStatus: "In-process UI",
  openInstantMcpTools: "Open Instant MCP Tools",
...
const zh: Record<keyof typeof en, string> = {
  ...
  networkProxy: "网络代理",
  liveMcpTools: "即时 MCP 工具",
  liveMcpToolsBody: "列出本地 Coding Tools 目录，并对真实工作区运行工具。结果来自无头运行时，不是预览数据。",
  liveMcpToolsMoved: "即时 MCP 工具已移到 Apps。打开即时 MCP 工具即可选择工作区、工具和参数，然后执行工具。",
  instantMcpTools: "即时 MCP 工具",
  instantMcpToolsHostBody: "工作区、工具、参数和执行工具在此页面。处理程序通过 codingTools.apps 在进程内运行；此应用不会开放监听端口，也不使用 Start。",
  instantMcpToolsHandlerStub: "现在可以调用 apps_list。instant-mcp-tools 的 app-handler 仍由 Bot GG 负责（inspect / listTools / runTool / listWorkspaces；kebab 别名 list-tools / run-tool）。此界面不会开放专用监听端口。",
  instantMcpToolsConnected: "已连接",
  instantMcpToolsInProcessStatus: "进程内画面",
  openInstantMcpTools: "打开即时 MCP 工具",
...
const ja: Record<keyof typeof en, string> = {
  ...
  liveMcpToolsMoved: "ライブ MCP ツールは Apps に移りました。Instant MCP Tools を開いてワークスペース、ツール、引数を選び、ツールを実行します。",
  instantMcpTools: "Instant MCP Tools",
  instantMcpToolsHostBody: "ワークスペース、ツール、引数、ツール実行はここにあります。ハンドラーは codingTools.apps 経由でプロセス内動作し、リスンポートや Start は使いません。",
  instantMcpToolsHandlerStub: "apps_list は既に呼び出せます。instant-mcp-tools の app-handler は Bot GG 担当（inspect / listTools / runTool / listWorkspaces；kebab 別名 list-tools / run-tool）です。この UI は専用リスンポートを開きません。",
  instantMcpToolsConnected: "接続済み",
  instantMcpToolsInProcessStatus: "プロセス内 UI",
  openInstantMcpTools: "Instant MCP Tools を開く",
