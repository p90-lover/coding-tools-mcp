...
const RELEASE_VERSION = "0.7.0-rc.10";
const STALE_VERSION = "0.7.0-rc.9";
...
test("the rc.10 manifest and every active package identity use one release version", () => {
  const manifest = JSON.parse(read("package.json"));
  assert.equal(manifest.version, RELEASE_VERSION);

  for (const relativePath of activeIdentityFiles) {
    const source = read(relativePath);
    assert.match(source, /0\.7\.0-rc\.10/, `${relativePath} must declare ${RELEASE_VERSION}`);
    assert.doesNotMatch(source, /0\.7\.0-rc\.9/, `${relativePath} still declares ${STALE_VERSION}`);
  }
});

test("the installer migration contract names the current rc.10 release", () => {
  const source = read("tests/installer-upgrade-migration.test.cjs");
  assert.match(source, /rc\.10 keeps the stable Electron installer identity/);
  assert.match(source, /manifest\.version, "0\.7\.0-rc\.10"/);
});
