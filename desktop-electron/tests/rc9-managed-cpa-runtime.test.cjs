...
test("the managed CPA adapter extracts safely and generates a private loopback-only runtime configuration", () => {
  ...
  assert.match(fs.readFileSync(requireFile("desktop-electron/electron/cpa-codex-long-run.cjs"), "utf8"), /keepalive-seconds: 15/);
...
test("a healthy managed CPA probe remains ready instead of being overwritten as starting", () => {
  ...
  assert.match(
    source,
    /mergedStatus = running\s*\?\s*\(service\.status === "error" \? "error" : service\.status === "ready" \? "ready" : "starting"\)/,
  );
});
