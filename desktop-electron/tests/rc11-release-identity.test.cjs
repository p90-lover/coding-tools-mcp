...
test("rc.11 exact-source runner publishes a new tag without moving frozen rc.8, rc.9, or rc.10 tags", () => {
  ...
  assert.match(workflow, /branches:\n      - main\n      - release\/codex-router-multiprovider-0\.7\.0-rc\.8/);
  assert.doesNotMatch(workflow, /RELEASE_TAG: v0\.7\.0-rc\.10/);

  const frozenRc10 = read(".github/workflows/codex-router-multiprovider-release-rc10.yml");
  assert.match(frozenRc10, /workflow_dispatch:/);
  assert.match(frozenRc10, /RELEASE_VERSION: 0\.7\.0-rc\.10/);
  assert.doesNotMatch(frozenRc10, /\n {2}push:\n {4}branches:/);

  const frozenRc8 = read(".github/workflows/codex-router-multiprovider-release-rc8.yml");
  assert.match(frozenRc8, /workflow_dispatch:/);
  assert.doesNotMatch(frozenRc8, /\n {2}push:\n {4}branches:\n {6}- release\/codex-router-multiprovider-0\.7\.0-rc\.8/);
