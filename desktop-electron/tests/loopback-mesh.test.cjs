...
test("this change does not rewrite CPA/Router keep-alive in provider-network", () => {
  ...
  assert.equal(diff, "");
  const source = fs.readFileSync(path.join(root, "electron/paseo-anneal-commandcode-long-run.cjs"), "utf8");
  assert.match(source, /WEEK_MS = 7 \* 24 \* 60 \* 60 \* 1000/);
  assert.match(source, /function shouldAbandonLongRun/);
  assert.match(source, /return false;/);
  assert.doesNotMatch(
    fs.readFileSync(path.join(root, "electron/cpa-codex-long-run.cjs"), "utf8"),
    /commandcode-proxy/,
  );
});
