...
const SERVICE_ENTRYPOINTS = Object.freeze(["start.mjs", "foreground-start.mjs"]);

function normalized(value) {
  return String(value || "").replaceAll("\\", "/").toLowerCase();
}

export function commandLineMatchesServiceEntrypoint(commandLine, sourceRoot) {
  const live = normalized(commandLine);
  if (!live) return false;
  return SERVICE_ENTRYPOINTS.some((name) => (
    live.includes(normalized(path.join(sourceRoot, "src", name)))
  ));
}
...
export function buildServiceProcessState({
  ...
  if (!commandLineMatchesServiceEntrypoint(liveCommandLine, sourceRoot)) return undefined;
  return {
...
export function serviceProcessOwns(
  ...
  if (!commandLineMatchesServiceEntrypoint(state.commandLine, state.sourceRoot)) return false;
  if (identity(pid, { platform }) !== state.processIdentity) return false;
  const liveCommandLine = commandLine(pid, { platform });
  return Boolean(
    liveCommandLine && commandLineMatchesServiceEntrypoint(liveCommandLine, state.sourceRoot),
  );
}
