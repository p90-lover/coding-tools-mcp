...
async function runPackagedLauncherProcess({
  ...
  for (;;) {
    if (state.spawnError) {
      ...
      throw new Error(failureMessage(`Packaged launcher could not start: ${state.spawnError.message}`, state));
    }
    ...
    if (state.exit) {
      if (marker) {
        return { marker, forcedTermination: false, exit: state.exit, stdout: state.stdout, stderr: state.stderr };
      }
    ...
    if (marker && Date.now() - markerSeenAt >= exitGraceMs) {
      await stopOwnedChild(child);
      return { marker, forcedTermination: true, exit: state.exit, stdout: state.stdout, stderr: state.stderr };
    }

    if (Date.now() - startedAt >= timeoutMs) {
      await stopOwnedChild(child);
      throw new Error(failureMessage(`Packaged launcher did not become ready within ${timeoutMs}ms`, state));
    }
    await delay(pollIntervalMs);
  }
}

module.exports = { runPackagedLauncherProcess };
