...
fn checked_directory(p: &Path) -> Result<PathBuf> {
    if !p.is_absolute() {
        bail!("Absolute directory required");
    }
    let real = p.canonicalize().context("Directory is unavailable")?;
    if !real.is_dir() {
        bail!("Directory is unavailable");
    }
    Ok(real)
}
fn profile(root: &Path) -> Result<PermissionProfile> {
    let mut paths = vec![root.to_path_buf()];
    // Explicit platform roots, not the whole current user's profile.
    ...
    let helper = std::env::current_exe()?
        .parent()
        .context("Helper location unavailable")?
        .canonicalize()?;
    paths.push(helper);
...
fn process(request: Request) -> Result<serde_json::Value> {
    ...
    let ready = sandbox::sandbox_setup_is_complete_with_settings(
        &home,
        &sandbox::WindowsSandboxProvisioningSettings::default(),
    );
    if request.operation == "status" {
        return Ok(
            json!({"ok":true,"ready":ready,"upstream_commit":UPSTREAM,"backend":"codex_windows_elevated","filesystem":"read_only","network":"restricted","model_calls":false}),
        );
    }
    ...
    if request.operation == "setup" {
        ...
        return Ok(
            json!({"ok":true,"ready":sandbox::sandbox_setup_is_complete_with_settings(&home,&sandbox::WindowsSandboxProvisioningSettings::default()),"upstream_commit":UPSTREAM,"model_calls":false}),
        );
    }
    if request.operation != "exec" || !ready {
        bail!(
            "Sandbox not prepared locally; no unsandboxed execution or automatic elevation is allowed"
        );
    }
    ...
    // Fully qualified executable only. PATH lookup and helper option injection are not accepted.
    let executable = PathBuf::from(&request.argv[0]);
    if !executable.is_absolute() || !executable.is_file() {
        bail!("Use an absolute executable path");
    }
    let result = sandbox::run_windows_sandbox_capture_for_permission_profile_elevated(
        sandbox::ElevatedSandboxProfileCaptureRequest {
            ...
            write_roots_override: Some(&[]),
            deny_read_paths_override: &[],
            deny_write_paths_override: &[],
        },
    )?;
    Ok(
        json!({"ok":result.exit_code==0,"exit_code":result.exit_code,"timed_out":result.timed_out,
...
fn main() {
    ...
    let response = match result {
        Ok(value) => value,
        Err(e) => {
            json!({"ok":false,"error":e.to_string(),"model_calls":false,"upstream_commit":UPSTREAM})
        }
    };
