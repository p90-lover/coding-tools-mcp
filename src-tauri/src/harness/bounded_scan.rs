...
use super::{
    model::{BaselineEntry, ProjectBaseline},
...
fn failure(code: &'static str, detail: &str) -> HarnessError {
    HarnessError::new(code,format!("{detail}; baseline not accepted. Inspect the selected project scope; do not repeatedly retry an unchanged failing scan."))
}
...
fn check(start: Instant) -> HarnessResult<()> {
    if start.elapsed() >= DEADLINE {
        Err(failure(
            "BASELINE_SCAN_LIMIT",
            "Eight-second cooperative scan deadline reached",
        ))
    } else {
        Ok(())
    }
...
pub(super) fn acquire(root: &Path) -> HarnessResult<Lease> {
    let mut active = ACTIVE
        ...
        .lock()
        .map_err(|_| failure("BASELINE_SCAN_BUSY", "Scan admission unavailable"))?;
    if !active.insert(root.to_path_buf()) {
        return Err(failure(
            "BASELINE_SCAN_BUSY",
            "This workspace already has a baseline scan in progress",
        ));
    }
    drop(active);
...
pub fn capture(root: &Path) -> HarnessResult<ProjectBaseline> {
    ...
    let iter = WalkDir::new(root)
        .follow_links(false)
        .into_iter()
        .filter_entry(|item| item.path() == root || !skipped(item.path(), root));
    for item in iter {
        check(started)?;
        visited += 1;
        ...
        let item =
            item.map_err(|_| failure("BASELINE_SCAN_INCOMPLETE", "Directory enumeration failed"))?;
        if item.path() == root || !item.file_type().is_file() {
            continue;
        }
        let path = item.path();
        let before = path
            .symlink_metadata()
            .map_err(|_| failure("BASELINE_SCAN_INCOMPLETE", "File metadata changed"))?;
        if before.file_type().is_symlink() || !before.is_file() {
            return Err(failure(
                "BASELINE_SCAN_INCOMPLETE",
                "Input type changed during scan",
            ));
        }
        if before.len() > FILE_BYTES || before.len() > TOTAL_BYTES.saturating_sub(total) {
            return Err(failure(
                "BASELINE_SCAN_LIMIT",
                "32 MiB file or 128 MiB aggregate input limit reached",
            ));
        }
        let canonical = path
            .canonicalize()
            .map_err(|_| failure("BASELINE_SCAN_INCOMPLETE", "Input disappeared"))?;
        if !canonical.starts_with(root) {
            return Err(failure(
                "BASELINE_SCAN_INCOMPLETE",
                "Input escaped workspace",
            ));
        }
        let mut input = File::open(&canonical)
            .map_err(|_| failure("BASELINE_SCAN_INCOMPLETE", "Input could not be opened"))?;
        ...
        loop {
            ...
            let n = input
                .read(&mut buffer)
                .map_err(|_| failure("BASELINE_SCAN_INCOMPLETE", "Input read failed"))?;
            if n == 0 {
                break;
            }
            length += n as u64;
            total += n as u64;
            if length > FILE_BYTES || total > TOTAL_BYTES {
                return Err(failure(
                    "BASELINE_SCAN_LIMIT",
                    "Input grew past scan budget",
                ));
            }
        ...
        let after = input.metadata().map_err(|_| {
            failure(
                "BASELINE_SCAN_INCOMPLETE",
                "Input metadata unavailable after read",
            )
        })?;
...
fn git_value(root: &Path, args: &[&str], started: Instant) -> HarnessResult<Option<String>> {
    ...
    let mut child = match cmd.spawn() {
        ...
        Err(_) => return Err(failure("BASELINE_SCAN_INCOMPLETE", "Git could not start")),
    };
        ...
        match child.try_wait() {
            Ok(Some(status)) => {
                ...
                child
                    ...
                    .map_err(|_| failure("BASELINE_SCAN_INCOMPLETE", "Git output unreadable"))?;
                if bytes.len() > 4096 {
                    return Err(failure(
                        "BASELINE_SCAN_INCOMPLETE",
                        "Git metadata exceeded 4 KiB",
                    ));
                }
            ...
            Ok(None)
                if git_started.elapsed() < GIT_METADATA_DEADLINE
