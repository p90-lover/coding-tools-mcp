...
fn error(code: &'static str, text: &str) -> HarnessError {
    HarnessError::new(code, text)
}
...
fn regular_file(root: &Path, relative: &Path) -> HarnessResult<Option<File>> {
    ...
    for part in relative.components() {
        if !matches!(part, std::path::Component::Normal(_)) {
            return Err(error("MONITOR_PATH", "Invalid metadata path"));
        }
        path.push(part);
        ...
        if meta.file_type().is_symlink() {
            return Err(error(
                "MONITOR_PATH",
                "Linked task metadata is not followed",
            ));
        }
        #[cfg(windows)]
        {
            ...
            if meta.file_attributes() & 0x400 != 0 {
                return Err(error(
                    "MONITOR_PATH",
                    "Reparse-point task metadata is not followed",
                ));
            }
        }
    }
    if !path
        .canonicalize()
        .map_err(|_| error("MONITOR_IO", "Metadata path unavailable"))?
        .starts_with(root)
    {
        return Err(error("MONITOR_PATH", "Metadata escaped the history root"));
    }
    if !fs::symlink_metadata(&path)
        .map_err(|_| error("MONITOR_IO", "Metadata unavailable"))?
        .is_file()
    {
        return Err(error(
            "MONITOR_PATH",
            "Expected a regular task metadata file",
        ));
    }
...
fn limited_json<T: for<'a> Deserialize<'a>>(
    mut file: File,
    cap: usize,
    budget: &mut usize,
) -> HarnessResult<T> {
    let len = file
        .metadata()
        .map_err(|_| error("MONITOR_IO", "Task metadata unavailable"))?
        .len();
    if len > cap as u64 || len > *budget as u64 {
        return Err(error(
            "MONITOR_LIMIT",
            "Task metadata exceeds the observation budget",
        ));
    }
    let mut bytes = Vec::with_capacity((len as usize).min(cap));
    (&mut file)
        .take(cap.min(*budget) as u64 + 1)
        .read_to_end(&mut bytes)
        .map_err(|_| error("MONITOR_IO", "Cannot read task metadata"))?;
    if bytes.len() > cap || bytes.len() > *budget {
        return Err(error(
            "MONITOR_LIMIT",
            "Task metadata grew past the observation budget",
        ));
    }
    *budget -= bytes.len();
    serde_json::from_slice(&bytes).map_err(|_| {
        error(
            "MONITOR_INVALID",
            "Task metadata is incomplete or malformed",
        )
    })
}
fn load_task(root: &Path, wid: &str, id: &str, budget: &mut usize) -> HarnessResult<Option<Task>> {
    if !valid_id(id) {
        return Err(error(
            "MONITOR_TASK_ID",
            "Task ID must contain exactly 32 hexadecimal characters",
        ));
    }
    let path = PathBuf::from("workspaces")
        .join(wid)
        .join("tasks")
        .join(format!("{id}.json"));
    let Some(file) = regular_file(root, &path)? else {
        return Ok(None);
    };
    let task: Task = limited_json(file, TASK_BYTES, budget)?;
    if task.id != id || task.workspace_id != wid {
        return Err(error(
            "MONITOR_SCOPE",
...
fn summary(t: &Task, details: bool) -> Value {
    ...
    if details {
        v["completed_steps"] = json!(t
            .completed_steps
            .iter()
            .take(64)
            .map(|s| short(s, 1024))
            .collect::<Vec<_>>());
        v["pending_steps"] = json!(t
            .pending_steps
            .iter()
            .take(64)
            .map(|s| short(s, 1024))
            .collect::<Vec<_>>());
        v["steps_truncated"] = json!(
            t.completed_steps.len() > 64
                || t.pending_steps.len() > 64
                || t.completed_steps
                    .iter()
                    .chain(&t.pending_steps)
                    .any(|s| s.len() > 1024)
        );
    }
    v
...
fn event(v: &Value, task: &str) -> HarnessResult<Value> {
    if v["task_id"] != task {
        return Err(error("MONITOR_SCOPE", "Event belongs to a different task"));
    }
    ...
    Ok(
        json!({"id":text(&v["id"],128),"task_id":task,"operation_id":text(&v["operation_id"],128),
...
fn events(root: &Path, wid: &str, task: &str, cursor: Option<&Cursor>) -> HarnessResult<Value> {
    ...
    let Some(mut file) = regular_file(root, &path)? else {
        return if cursor.is_some() {
            Err(error(
                "MONITOR_CURSOR",
                "History changed; return to latest events",
            ))
        } else {
            Ok(json!({"events":[],"next_cursor":null,"partial_tail":false,"warnings":[]}))
        };
    };
    let length = file
        .metadata()
        .map_err(|_| error("MONITOR_IO", "Cannot inspect event history"))?
        .len();
    let end = cursor.map(|c| c.before).unwrap_or(length);
    if end > length || end > 9_007_199_254_740_991 {
        return Err(error(
            "MONITOR_CURSOR",
            "Event cursor is outside current history",
        ));
    }
    if cursor.is_some() && end > 0 {
        file.seek(SeekFrom::Start(end - 1))
            .map_err(|_| error("MONITOR_IO", "Cannot seek history"))?;
        let mut b = [0];
        file.read_exact(&mut b)
            .map_err(|_| error("MONITOR_IO", "History changed"))?;
        if b[0] != b'\n' {
            return Err(error(
                "MONITOR_CURSOR",
                "Event cursor must point to a complete record boundary",
            ));
        }
    }
    ...
    file.take(end - start)
        .read_to_end(&mut bytes)
        .map_err(|_| error("MONITOR_IO", "Cannot read event history"))?;
    let mut pos = 0;
    ...
    if start > 0 {
        pos = bytes
            ...
            .map(|i| i + 1)
            .unwrap_or(bytes.len());
        warnings.push("Older events outside this bounded window are not loaded".into());
    }
    ...
    while pos < bytes.len() {
        ...
        if n > EVENT_BYTES {
            warnings.push(
                "An oversized event was omitted from the display; original is retained".into(),
            );
            continue;
        }
        let v: Value = match serde_json::from_slice(&bytes[begin..begin + n]) {
            Ok(v) => v,
            Err(_) => {
                warnings.push("A malformed event was omitted; original is retained".into());
                continue;
            }
        };
    ...
    let next = before.map(|before| Cursor {
    ...
    warnings.dedup();
    Ok(
        json!({"events":rows.into_iter().map(|(_,v)|v).collect::<Vec<_>>(),"next_cursor":next,"partial_tail":partial_tail,"warnings":warnings}),
    )
}
...
pub fn read(
    ...
    let workspace_root = workspace_root
        .canonicalize()
        .map_err(|_| error("MONITOR_WORKSPACE", "Selected workspace is unavailable"))?;
    let wid = workspace_id(&workspace_root);
    if task_id.is_some_and(|id| !valid_id(id)) {
        return Err(error("MONITOR_TASK_ID", "Invalid task ID"));
    }
    if cursor.is_some_and(|c| c.workspace_id != wid || Some(c.task_id.as_str()) != task_id) {
        return Err(error(
            "MONITOR_SCOPE",
            "Event cursor belongs to a different workspace or task",
        ));
    }
    ...
    if fs::symlink_metadata(history_root)
        .map_err(|_| error("MONITOR_IO", "History unavailable"))?
        .file_type()
        .is_symlink()
    {
    ...
    let index: Option<Index> = regular_file(&root, &index_path)?
        .map(|f| limited_json(f, INDEX_BYTES, &mut budget))
        .transpose()?;
    let mut ids = Vec::<String>::new();
    if let Some(index) = index {
        if let Some(id) = index.active_task_id {
            ids.push(id);
        }
        ids.extend(index.recent_task_ids);
    }
    let mut unique = HashSet::new();
    ids.retain(|id| unique.insert(id.clone()));
    ids.truncate(RECENT);
    let selected = task_id
        .map(|id| load_task(&root, &wid, id, &mut budget))
        .transpose()?
        .flatten();
    if task_id.is_some() && selected.is_none() {
        return Err(error(
            "MONITOR_NOT_FOUND",
            "Task not found in the selected workspace",
        ));
    }
    let mut tasks = Vec::new();
    for id in ids {
        if Some(id.as_str()) == task_id {
            tasks.push(summary(selected.as_ref().expect("selected task"), false));
            continue;
        }
        match load_task(&root, &wid, &id, &mut budget) {
            ...
            Err(e) if e.code() == "MONITOR_LIMIT" => {
                warnings.push("Recent task list reached its read budget".into());
                break;
            }
    ...
    let mut view = if let Some(id) = task_id {
        events(&root, &wid, id, cursor)?
    } else {
        empty
    };
    warnings.extend(
    ...
    warnings.sort();
    warnings.dedup();
    view["tasks"] = json!(tasks);
    view["task"] = selected
        .as_ref()
        .map(|t| summary(t, true))
        .unwrap_or(Value::Null);
    view["warnings"] = json!(warnings);
    view["recent_limit"] = json!(RECENT);
    view["history_workspace_id"] = json!(wid);
    view["read_only"] = json!(true);
    view["project_baseline_scanned"] = json!(false);
    Ok(view)
}
