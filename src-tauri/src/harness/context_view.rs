...
impl Write for Budget {
    fn write(&mut self, bytes: &[u8]) -> io::Result<usize> {
        if bytes.len() > self.remaining {
            return Err(io::Error::other("context byte budget"));
        }
...
fn fits(value: &Value, limit: usize) -> bool {
    serde_json::to_writer(&mut Budget { remaining: limit }, value).is_ok()
}
pub fn render(task: Value, events: Value, max_bytes: usize) -> Value {
    ...
    if let Some(entries) = view
        .pointer_mut("/task/baseline/entries")
        .and_then(Value::as_array_mut)
    {
        omitted.insert("baseline_entries".into(), json!(entries.len()));
        entries.clear();
    }
    for key in ["completed_steps", "pending_steps"] {
        if let Some(steps) = view["task"].get_mut(key).and_then(Value::as_array_mut) {
            if steps.len() > 16 {
                omitted.insert(key.into(), json!(steps.len() - 16));
                steps.truncate(16);
            }
        }
    }
            ...
            for value in values {
                if let Some(text) = value.as_str() {
                    if text.len() > 512 {
                        ...
                        *value = json!(format!("{} [view truncated]", &text[..end]));
                        shortened.push(key);
                    }
                }
            }
    ...
    loop {
        ...
        if shown > 0 {
            view["events"].as_array_mut().unwrap().pop();
            continue;
        }
        ...
        view["task"] = summary;
        view["task_summary_only"] = json!(true);
        if fits(&view, limit - 64) {
            return view;
        }
