...
pub(super) const COMMAND_RUNTIME_SHA256: Option<&str> =
    option_env!("CODING_TOOLS_COMMAND_NATIVE_SHA256");
fn default_timeout() -> u64 {
    5000
}
fn validate(request: &CommandRequest) -> Result<()> {
    if !token(&request.request_id)
        || !(100..=10000).contains(&request.timeout_ms)
    ...
    {
        return Err(
            "Use a unique request_id, 1–32 bounded argv strings and a 100–10000 ms timeout".into(),
        );
    }
    Ok(())
}
fn reserve_command(
    ...
    if !live || !ready {
        return Err("Native session is not connected".into());
    }
    if !options.allow_command_execution {
        return Err(
            "Native commands need their own local consent; model consent is not command consent"
                .into(),
        );
    }
...
impl Hub {
    ...
    pub fn admit_command(&self, request: CommandRequest) -> Result<CommandTicket> {
        ...
        let pinned = COMMAND_RUNTIME_SHA256.ok_or(
            "This build has no verified native command runtime digest; no command submitted",
        )?;
...
impl CommandTicket {
    ...
    fn execute(&self) -> Result<Value> {
        ...
        let value = self.bridge.rpc(
            "command/exec",
            json!({"command":self.request.argv,
            "cwd":self.bridge.root,"permissionProfile":":read-only",
            "timeoutMs":self.request.timeout_ms,"outputBytesCap":16384,
        ...
        let stdout = value["stdout"]
            .as_str()
            .ok_or("Native command response omitted stdout")?;
        let stderr = value["stderr"]
            .as_str()
            .ok_or("Native command response omitted stderr")?;
        Ok(
            json!({"ok":true,"request_id":self.request.request_id,"exit_code":exit,
...
mod tests {
    ...
    fn native_043_command_consent_bounds_and_replay_are_independent() {
        ...
        let r = CommandRequest {
            ...
            argv: vec!["tool".into(), "literal arg".into()],
            timeout_ms: 1000,
        };
        ...
        options.allow_model_usage = false;
        options.allow_command_execution = true;
        assert!(reserve_command(&mut memory, &options, &r, true, true)
            .unwrap()
            .is_none());
