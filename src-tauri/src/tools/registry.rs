...
pub const P0_TOOLS: &[(&str, &str, &str, bool, bool, bool)] = &[
    ...
    ("sandbox_exec", "Read-only native sandbox execution", "Run an absolute executable and literal arguments under the prepared native Windows sandbox: blocked workspace writes, restricted network and private desktop. Read access follows the sandbox account Windows ACLs; shared files outside the workspace may remain readable, so this is not a read-path whitelist. Requires prior local setup; never falls back to unsandboxed execution or launches a Codex agent.", false, false, false),
