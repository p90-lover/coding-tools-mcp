...
pub const P0_TOOLS: &[(&str, &str, &str, bool, bool, bool)] = &[
    ...
    ("codex_agent_control", "Control opted-in native Codex agent", "Submit start/send/review/compact or interrupt/close to a locally enabled native App Server. Model operations can spend provider quota. Unique request_id is mandatory. Matching completed receipts replay only while retained in bounded RAM (90-minute age retention with oldest-first pressure eviction); after expiry or eviction the same ID may submit again, so use a fresh request_id for every new intended operation. Pending or unknown outcomes are never evicted to make room. Only owned threads, no remote enable, approval escalation or arbitrary RPC. An acknowledgment is not task completion.", false, true, true),
