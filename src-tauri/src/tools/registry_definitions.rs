...
pub const P0_TOOLS: &[(&str, &str, &str, bool, bool, bool)] = &[
    ...
    ("workflow_list", "Read workspace workflow", "Read this workspace's board and durable agent missions. Optional refresh_source performs observation only; it never resubmits a mission. Credentials and provider consent are configured locally.", true, false, false),
    ("workflow_update", "Update workspace workflow", "Update scoped board records, prepare a mission, control its owned Paseo or Anneal record, or attest an independent review. Agent create/start/resume can invoke a separately approved external provider and consume quota. Local provider consent is required; no credential or endpoint is accepted from MCP. Uncertain operations are never automatically replayed.", false, false, true),
