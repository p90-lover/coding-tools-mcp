...
pub fn for_tool(name:&str)->Option<Value>{
    let (required,properties)=match name {
        "read_file"=>(json!(["path","content","encoding","truncated"]),json!({
        ...
        "operation_log"=>(json!(["operations","next_cursor"]),json!({
    ...
    Some(json!({"type":"object","required":["ok"],"properties":properties,"additionalProperties":true,
