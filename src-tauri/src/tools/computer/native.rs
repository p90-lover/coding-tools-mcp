...
pub fn validate_identity(target: &Target, foreground: bool) -> Result<()> {
    ...
    if target.pid == unsafe { GetCurrentProcessId() } && !cfg!(test) {
        return Err(error(
            "PROTECTED_TARGET",
...
pub fn validate_target(target: &Target, foreground: bool) -> Result<Bounds> {
    ...
    let w = xcap::Window::all()
        .map_err(|_| failure("Cannot verify target geometry"))?
        .into_iter()
    ...
    match (x, y, width, height) {
        (Ok(x), Ok(y), Ok(width), Ok(height)) if width > 0 && height > 0 => Ok(Bounds {
            x,
            y,
            width,
            height,
...
mod fixture_test {
    ...
    fn computer_native_fixture_round_trip() {
        ...
        super::super::local_arm(root, f.target.clone(), 60).expect("arm own fixture");
        ...
        while start.elapsed() < Duration::from_secs(3) {
            ...
            actual = String::from_utf16_lossy(&buffer[..n as usize]);
