...
pub fn describe(profile: &str) -> Value {
    ...
    let hidden: Vec<&str> = all.iter().filter_map(|t| t["name"].as_str())
        .filter(|n| !names.contains(n)).collect();
    let read_only = tools.iter().filter(|t| t["annotations"]["readOnlyHint"] == true).count();
...
mod tests {
    ...
    fn catalog_exposure_counts_fingerprint_and_unknown_client_are_honest() {
        ...
        assert!(full["advertised_names"].as_array().unwrap().iter().any(|n|n=="workflow_update"));
        for profile in ["core","read-only","advanced","compat-readonly-all"] {
            ...
            assert_eq!(report["advertised_names"],Value::Array(actual.iter().map(|t|t["name"].clone()).collect()));
            assert_eq!(report["registered_count"].as_u64().unwrap(),
                report["advertised_count"].as_u64().unwrap()+report["hidden_by_profile"].as_array().unwrap().len() as u64);
        ...
        assert!(core["advertised_count"].as_u64().unwrap()<full["advertised_count"].as_u64().unwrap());
        assert!(core["hidden_by_profile"].as_array().unwrap().iter().any(|n|n=="start_task"));
    ...
    fn catalog_exposure_live_profile_matches_rpc_without_restart_or_privilege_escalation() {
        let root=std::env::current_dir().unwrap().join("aiTemp/catalog-exposure")
        ...
        super::super::live_policy::commit_updates(vec![(ctx.clone(),policy.clone(),"core".into())],||Ok(())).unwrap();
        let core=describe_current(&ctx.for_request().unwrap());
        super::super::live_policy::commit_updates(vec![(ctx.clone(),policy,"full".into())],||Ok(())).unwrap();
        let full=describe_current(&ctx.for_request().unwrap());
        let list=crate::mcp::server::handle_request(&ctx,&json!({"jsonrpc":"2.0","id":1,"method":"tools/list"}));
        assert_eq!(full["advertised_count"],list["result"]["tools"].as_array().unwrap().len());
        assert_eq!(full["permission_mode"],"read-only");
        assert_eq!(full["screen_capture_allowed"],false);
        assert_ne!(core["catalog_sha256"],full["catalog_sha256"]);
        let info=crate::mcp::server::handle_request(&ctx,&json!({"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"server_info","arguments":{}}}));
        assert_eq!(info["result"]["structuredContent"]["tool_catalog"],full);
        let denied=crate::mcp::server::handle_request(&ctx,&json!({"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"apply_patch","arguments":{"patch":"*** Begin Patch\n*** Add File: must-not-exist.txt\n+denied\n*** End Patch\n"}}}));
        ...
        super::super::live_policy::commit_updates(vec![(ctx.clone(),writable,"advanced".into())],||Ok(())).unwrap();
        let after=describe_current(&ctx.for_request().unwrap());
        assert_eq!(after["catalog_sha256"],full["catalog_sha256"],"Permission-only changes must not change tool definitions");
