...
use axum::http::{header::{ACCEPT, ALLOW, WWW_AUTHENTICATE}, HeaderMap, HeaderValue, StatusCode};
use axum::response::{IntoResponse, Response};
use axum::Json;
use serde_json::{json, Value};
use super::{ListenerState, mcp_discovery, require_mcp_auth};

/// Keep the desktop's legacy JSON health probe, but do not impersonate an SSE stream.
pub(super) async fn get_handler(State(state): State<ListenerState>, headers: HeaderMap) -> Response {
    let wants_stream = headers.get_all(ACCEPT).iter().filter_map(|h| h.to_str().ok())
        .flat_map(|h| h.split(','))
        .any(|h| h.split(';').next().unwrap_or("").trim().eq_ignore_ascii_case("text/event-stream"));
    if wants_stream {
        if let Some(response) = require_mcp_auth(&state, &headers) { return response; }
...
pub(super) fn oauth_challenge(mut response: Response, server_url: &str) -> Response {
    if response.status() != StatusCode::UNAUTHORIZED { return response; }
    ...
    let Ok(value) = HeaderValue::from_str(&challenge) else {
        return (StatusCode::SERVICE_UNAVAILABLE, "OAuth discovery configuration is invalid").into_response();
    };
...
pub(super) fn early_response(body: &Value) -> Option<Response> {
    ...
    let valid = body.is_object() && body["jsonrpc"] == "2.0"
    ...
    if valid && body.get("id").is_some_and(|id| id.is_string() || id.is_i64() || id.is_u64()) {
        return None;
    }
