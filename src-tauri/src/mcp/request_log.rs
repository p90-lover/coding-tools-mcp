...
fn bounded_line(value: &str) -> (String, bool) {
    ...
    let mut end = value.len().min(if truncated {
        LINE_BYTES - 20
    } else {
        LINE_BYTES
    });
...
pub(crate) fn append_profile_log(profile: &str, file: &str, line: &str) {
    TRACE
        .get_or_init(|| TraceQueue::new(CAPACITY, crate::tunnel::append_profile_log))
        .append(profile, file, line);
}
...
mod tests {
    ...
    fn resilience_trace_queue_bounds_order_and_failure_are_visible() {
        ...
        q.append("profile", "trace.log", "first");
        started.recv_timeout(Duration::from_secs(2)).unwrap();
        q.append("profile", "trace.log", "second");
        q.append("profile", "trace.log", "third");
        for _ in 0..1000 {
            q.append("profile", "trace.log", "overflow");
        }
        assert_eq!(q.counters.pending.load(Ordering::Relaxed), 3);
        assert_eq!(q.counters.dropped.load(Ordering::Relaxed), 1000);
        resume.send(()).unwrap();
        let until = std::time::Instant::now() + Duration::from_secs(3);
        while q.counters.pending.load(Ordering::Relaxed) > 0 && std::time::Instant::now() < until {
        ...
        let q = TraceQueue::new(1, |_, _, _| panic!("synthetic sink failure"));
        q.append("p", "f", "safe");
