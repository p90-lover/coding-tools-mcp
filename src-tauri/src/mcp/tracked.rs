...
fn annotate(mut response: Value, id: &str, runtime: &str) -> Value {
    let receipt = json!({"operation_id":id,"runtime_id":runtime,"query_tool":TOOL,"safe_to_retry":false});
    if let Some(result) = response.get_mut("result").and_then(Value::as_object_mut) {
        let meta = result.entry("_meta").or_insert_with(|| json!({}));
        if let Some(meta) = meta.as_object_mut() { meta.insert(META_KEY.into(), receipt); }
    } else if let Some(error) = response.get_mut("error").and_then(Value::as_object_mut) {
        let data = error.entry("data").or_insert_with(|| json!({}));
        if let Some(data) = data.as_object_mut() { data.insert("operation".into(), receipt); }
    }
    response
}
fn http(status: StatusCode, value: Value, id: &str) -> Response {
    let mut response = (status, Json(value)).into_response();
    if let Ok(id) = HeaderValue::from_str(id) { response.headers_mut().insert("x-mcp-operation-id", id); }
    response
}
...
struct CompletionGuard {
    store: Arc<OperationStore>, id: String, request_id: Value, record: Recorder, finished: bool,
}
impl Drop for CompletionGuard {
    fn drop(&mut self) {
        if !self.finished {
            self.store.finish(&self.id, &worker_error(&self.request_id), true);
            (self.record)(&format!("[rpc] worker_failed operation_id={} id={} outcome=unknown", self.id, self.request_id));
        }
    }
}

pub async fn execute(
    store: Arc<OperationStore>, body: Value, revision: u64, wait: Duration,
    work: impl FnOnce() -> Value + Send + 'static, record: Recorder,
) -> Response {
    ...
    (record)(&format!("[rpc] admitted operation_id={} id={} method={} tool={}", id, request_id, body["method"], body.pointer("/params/name").unwrap_or(&Value::Null)));
    let mut completion = CompletionGuard { store:store.clone(), id:id.clone(), request_id:request_id.clone(), record:record.clone(), finished:false };
    let worker = tokio::task::spawn_blocking(move || {
        completion.store.started(&completion.id);
        (completion.record)(&format!("[rpc] started operation_id={} id={}", completion.id, completion.request_id));
        ...
        (completion.record)(&format!("[rpc] {} operation_id={} id={} rpc_error={} tool_error={}",
            if failed { "worker_failed" } else { "completed" }, completion.id, completion.request_id,
            response.get("error").is_some(), response.pointer("/result/isError")==Some(&Value::Bool(true))));
        response
    });
    match tokio::time::timeout(wait, worker).await {
        Ok(Ok(response)) => http(StatusCode::OK, response, &id),
        Ok(Err(_)) => http(StatusCode::INTERNAL_SERVER_ERROR, annotate(worker_error(&request_id), &id, store.runtime_id()), &id),
        Err(_) => {
            (record)(&format!("[rpc] http_wait_expired operation_id={} id={} execution_not_cancelled=true", id, request_id));
            http(StatusCode::REQUEST_TIMEOUT, json!({"jsonrpc":"2.0","id":request_id,"error":{
...
pub async fn query(work: impl FnOnce() -> Value + Send + 'static) -> Response {
    ...
    let permit = READERS.get_or_init(|| Arc::new(Semaphore::new(4))).clone().try_acquire_owned();
    let Ok(permit) = permit else { return (StatusCode::SERVICE_UNAVAILABLE, "Operation lookup capacity reached").into_response(); };
    let worker = tokio::task::spawn_blocking(move || { let _permit=permit; work() });
    match tokio::time::timeout(Duration::from_secs(5), worker).await {
        Ok(Ok(response)) => Json(response).into_response(),
        _ => (StatusCode::SERVICE_UNAVAILABLE, "Read-only operation lookup unavailable; no original operation was retried").into_response(),
    }
}
