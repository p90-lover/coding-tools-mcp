...
**Release line: `v0.4.6` — scoped OAuth popup origin repair.** Read the [release notes](docs/releases/v0.4.6.md) and the published release's validation evidence. A version in source code is not proof that a build passed. This is a release candidate, not a certified security sandbox.
...
Get `Coding.Tools.MCP_0.4.6_x64-setup.exe` from the [versioned release page](https://github.com/p90-lover/coding-tools-mcp/releases/tag/v0.4.6). This focused Windows release provides SHA-256 checksums, source provenance and validation logs. The prior `v0.4.3-rc.1` release retains the Apple Silicon `.dmg`. Windows is publisher-unsigned; macOS uses ad-hoc signing and is not notarized. Verify the source and checksum before opening a downloaded installer.
