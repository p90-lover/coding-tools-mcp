...
Get the currently published installer and `SHA256SUMS` from the [Latest stable release](https://github.com/p90-lover/coding-tools-mcp/releases/latest). The actual release asset list identifies which platforms completed verification. Windows is publisher-unsigned; macOS uses ad-hoc signing and is not notarized. Verify the source and checksum before opening an installer. Finish or checkpoint active work before updating the desktop, then refresh the client's tool catalogue for the new timeout schema.
...
CPA, Codex Router, CommandCode, Paseo, and Anneal live as modules under `modules/`. Coding Tools is the single host: call each module through `codingTools.apps` instead of opening those apps’ standalone GUIs. [Module layout and API calls](modules/README.md)
