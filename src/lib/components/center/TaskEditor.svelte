<script lang="ts">
 ...
 import {saveTask,type LocalTask,type TaskDraft} from '$lib/control-center/api';
...
<dialog bind:this={dialog} class="cc-task-dialog" oncancel={(event)=>{event.preventDefault();if(!busy)onclose();}} aria-labelledby="task-editor-title">
 <form onsubmit={(e)=>{e.preventDefault();void save();}}>
  ...
  <div class="cc-dialog-content">
   ...
   <div class="cc-form-row"><label class="cc-field">{$locale==='en'?'Workspace':'工作區'}<select class="cc-input" aria-label={$locale==='en'?'Workspace':'工作區'} bind:value={draft.workspace_id} disabled={Boolean(task)} required><option value="" disabled>{$locale==='en'?'Choose a workspace':'選擇工作區'}</option>{#each $workspaces as w}<option value={w.id}>{w.name}</option>{/each}</select></label><label class="cc-field">{$locale==='en'?'Priority':'優先次序'}<select class="cc-input" aria-label={$locale==='en'?'Priority':'優先次序'} bind:value={draft.priority}><option value="low">{$locale==='en'?'Low':'低'}</option><option value="normal">{$locale==='en'?'Normal':'一般'}</option><option value="high">{$locale==='en'?'High':'高'}</option></select></label></div>
   ...
   <label class="cc-field">{$locale==='en'?'Status':'狀態'}<select class="cc-input" aria-label={$locale==='en'?'Status':'狀態'} bind:value={draft.status}>{#each columns as c}<option value={c.status}>{$locale==='en'?c.label:c.zh}</option>{/each}</select></label>
