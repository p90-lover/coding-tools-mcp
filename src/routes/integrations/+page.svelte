<script lang="ts">
 ...
 import { Copy, Plug, RefreshCw, ShieldCheck, Unplug, ExternalLink } from '@lucide/svelte';
 ...
 async function copyAnnealForward(){await navigator.clipboard.writeText(annealForward);copiedForward=true;}
</script>
<section class="cc-page">
 ...
 {#if source==='anneal'}<p class="cc-help">{t($locale,'Anneal’s upstream is macOS/Linux, not native Windows. Copy a local forward if the API already runs on another host:','Anneal 上游支援 macOS／Linux，不是原生 Windows。若 API 已在其他主機運行，可複製本機轉送：')} <code>{annealForward}</code></p>
 <button type="button" class="cc-button ghost" onclick={()=>void copyAnnealForward()}><Copy size={15}/>{t($locale,copiedForward?'Copied port-forward command':'Copy port-forward command',copiedForward?'已複製連接埠轉送命令':'複製連接埠轉送命令')}</button>{/if}
 <div class="cc-button-row"><button class="cc-button primary" disabled={$integrationBusy[source]}><RefreshCw size={15} class={$integrationBusy[source]?'cc-spin':''}/>{t($locale,$integrationBusy[source]?'Reading…':$snapshots[source]?'Refresh snapshot':'Connect & read',$integrationBusy[source]?'讀取中…':$snapshots[source]?'更新狀態':'連接並讀取')}</button>{#if $snapshots[source]||$integrationBusy[source]}<button type="button" class="cc-button ghost" onclick={()=>clearIntegration(source)}><Unplug size={15}/>{t($locale,'Clear view','清除顯示')}</button>{/if}</div>
 </form>
