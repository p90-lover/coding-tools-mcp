...
import { RELEASE_TAG, RELEASE_VERSION } from './verify-source-scope-v0.6.0.mjs';
import { verifyReleaseAssets } from './verify-assets-v0.6.0.mjs';
...
function headers(token, extra = {}) {
  return {
    ...
    'user-agent': 'coding-tools-electron-release/0.6.0',
    ...extra,
  };
}
...
function validateReleaseRecord(release, { tag, requireDraft = null }) {
  ...
  assertValue(release?.prerelease === false, 'RELEASE_IS_PRERELEASE', String(release?.prerelease));
...
async function main() {
  ...
  const result = await publishVerifiedRelease({ ...options, notes });
