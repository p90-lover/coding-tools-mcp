...
- Generated renderer output is retained under `aiTemp/Trash` before source-integrity checks, so verification does not modify or delete tracked source.
...
- 進行 source-integrity 檢查之前，產生嘅 renderer output 會保留到 `aiTemp/Trash`，驗證流程唔會修改或刪除 tracked source。
