...
Negotiation now explicitly supports the 2025-03-26, 2025-06-18 and 2025-11-25 handshake revisions. Catalog responses carry definition-only fingerprints; permission-only changes do not alter them. `listChanged` remains false without a push stream, and a protocol fallback does not by itself prove why ChatGPT has no tools. The five v0.4.1 tools are included in every normalized profile. See the repair guide for exact diagnostic fields.
