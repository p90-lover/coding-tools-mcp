...
class RuntimeSupervisor {
  ...
  async startConfigured() {
    ...
    if (!tunnelOnly && config.releaseVersion !== this.app.getVersion() && !(function () {
      try {
        const roots = [];
        if (typeof this.installedRuntimeRoot === "string" && this.installedRuntimeRoot) roots.push(this.installedRuntimeRoot);
        if (typeof this.sourceRoot === "string" && this.sourceRoot) roots.push(this.sourceRoot);
        try { if (typeof this.runtimeRootProvider === "function") { const r = this.runtimeRootProvider(); if (r) roots.push(r); } } catch {}
        for (const root of roots) {
          for (const rel of ["package.json", path.join("app", "package.json")]) {
            const pkgPath = path.join(root, rel);
            if (!fs.existsSync(pkgPath)) continue;
            const ver = JSON.parse(fs.readFileSync(pkgPath, "utf8")).version;
            if (typeof ver === "string" && ver.trim() === config.releaseVersion) return true;
          }
        }
      } catch {}
      // preferLocal / app-modules offline: do not block Install on launcher↔runtime version namespace split
      try {
        const res = typeof process !== "undefined" ? process.resourcesPath : "";
        for (const name of ["preferLocal.json", "pack-stamp.json"]) {
          const p = path.join(res || "", "app-modules", name);
          if (fs.existsSync(p) && JSON.parse(fs.readFileSync(p, "utf8")).preferLocal) return true;
        }
      } catch {}
      return false;
    }.call(this))) {
