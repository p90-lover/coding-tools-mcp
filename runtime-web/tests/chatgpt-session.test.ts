...
function reasoningPicker(options: { max?: string; delay?: number; missing?: boolean } = {}) {
  ...
  const modelRows = {
    count: async () => chips.length,
    first() { return this; },
    waitFor: async () => {},
    nth: (index: number) => ({
      innerText: async () => chips[index]?.label ?? "",
      getAttribute: async () => null,
      click: async () => {},
    }),
  };
...
test.each([
  ...
  ["Latest", "latest"],
  ["GPT-6 Pro", undefined],
] as const)("maps ChatGPT model-chip label %j to pin %j", (label, pin) => {
  expect(matchChatGptWebModelPin(label)).toBe(pin);
});
