...
test('rejects a Tauri-only release path for the Electron OAuth product', () => {
  const result = auditOAuthReleaseContract({
    ...
    tauriConfig: { version: '0.4.10', identifier: 'com.codingtools.mcp.desktop' },
    releaseWorkflow: `
...
test('accepts the actual Electron artifacts directory through the CLI', () => {
  ...
  const fixture = path.join(path.dirname(testDir), 'aiTemp', `oauth-release-contract-positive-${process.pid}`);
  ...
  fs.writeFileSync(path.join(fixture, 'package.json'), JSON.stringify({ version: '0.4.10' }));
  fs.writeFileSync(path.join(fixture, 'desktop-electron', 'package.json'), JSON.stringify({
    version: '0.6.0',
    build: { appId: 'dev.codingtools.fullharness', directories: { output: 'release' } },
  }));
  fs.writeFileSync(path.join(fixture, 'src-tauri', 'tauri.conf.json'), JSON.stringify({
    version: '0.4.10',
    identifier: 'com.codingtools.mcp.desktop',
  }));
  fs.writeFileSync(path.join(fixture, '.github', 'workflows', 'release.yml'), `
...
test('rejects an Electron package with the wrong OAuth app identity', () => {
  const result = auditOAuthReleaseContract({
    ...
    electronPackage: {
      version: '0.6.0',
      build: { appId: 'com.example.different-product', directories: { output: 'release' } },
    },
    tauriConfig: { version: '0.4.10', identifier: 'com.codingtools.mcp.desktop' },
    releaseWorkflow: `
      - run: node -e "require('./desktop-electron/package.json')"
  ...
  assert.equal(result.ok, false);
  assert.deepEqual(result.errors.map(({ code }) => code), ['ELECTRON_APP_ID_MISMATCH']);
