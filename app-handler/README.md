...
| `instant-mcp-tools` | [`app-handler/instant-mcp-tools/`](instant-mcp-tools/) | [`handler.cjs`](instant-mcp-tools/handler.cjs) → [`handlers.cjs`](instant-mcp-tools/handlers.cjs) | same; aliases `list-tools` / `tools` → `listTools`, `run-tool` / `callTool` → `runTool` | [`McpLiveToolsPanel`](../desktop-electron/src/features/McpLiveToolsPanel.tsx) on the MCP page (no dedicated listen port) |
...
| `instant-mcp-tools` | inspect (+ no-op start/stop/restart/repair/install; Start is not required) | listTools/list-tools/tools, runTool/run-tool/callTool, listWorkspaces |
